...
proof (coinduction rule: bisim_coinduct)
  ...
  then show ?case
  proof -
    have "\<exists>op2'. step (Inp p x) (comp_op (\<lambda>_. None) (\<lambda>_. []) (sink_op::('a, 'c, 'e) op) (sink_op::('b, 'd, 'e) op)) op2' \<and> \<B> (\<lambda>op1 op2. op1 = sink_op \<and> op2 = comp_op (\<lambda>_. None) (\<lambda>_. []) sink_op sink_op) sink_op op2'"
      if "p \<notin> defaults"
      for p :: "'a + 'b"
        and x :: 'e
    proof (cases p)
    ...
    qed
    then show ?thesis
      using SIM1 by (auto elim !: step_sink_op)
  qed
next
  case SIM2
  then show ?case
