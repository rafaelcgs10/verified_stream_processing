...
corec comp_op :: "('op1 \<rightharpoonup> 'ip2) \<Rightarrow> ('ip2 \<Rightarrow> 'd buf) \<Rightarrow>
  ...
  | Choice ops \<Rightarrow> cimage (\<lambda> op. Base_aux (buf, op, op2)) (cfilter (Not o finished) ops))
  (case op2 of
   ...
   | Choice ops \<Rightarrow> cimage (\<lambda> op. Base_aux (buf, op1, op)) (cfilter (Not o finished) ops)
   )))"

lemma comp_op_code:
  "comp_op wire buf op1 op2 = Choice (cUn (case op1 of
    Read p f \<Rightarrow> {|Read (Inl p) (\<lambda> x. comp_op wire buf (f x) op2)|}
  | Write op1' p x \<Rightarrow> (case wire p of None \<Rightarrow> {|Write (comp_op wire buf op1' op2) (Inl p) x|} | Some q \<Rightarrow> {|comp_op wire (BENQ q x buf) op1' op2|})
  | Choice ops \<Rightarrow> cimage (\<lambda> op. comp_op wire buf op op2) (cfilter (Not o finished) ops))
  (case op2 of
  ...
  | Choice ops \<Rightarrow> cimage (\<lambda> op. comp_op wire buf op1 op) (cfilter (Not o finished) ops)))"
...
lemma comp_op_simps[simp]:
  ...
  "comp_op wire buf (Read p1 f1) (Choice op2s) = 
    Choice (cinsert (Read (Inl p1) (\<lambda>y. comp_op wire buf (f1 y) (Choice op2s))) (cimage (\<lambda> op. comp_op wire buf (Read p1 f1) op) (cfilter (Not o finished) op2s)))"
  ...
  "comp_op wire buf (Write op1 q1 x1) (Choice op2s) =
     Choice (cinsert (case wire q1 of None \<Rightarrow> Write (comp_op wire buf op1 (Choice op2s)) (Inl q1) x1
      ...
      (cimage (\<lambda> op. comp_op wire buf (Write op1 q1 x1) op) (cfilter (Not o finished) op2s)))"
  "comp_op wire buf (Choice op1s) (Read p2 f2) =
    Choice (cUn (if p2 \<in> ran wire then (if buf p2 = [] then cempty else csingle (comp_op wire (buf(p2 := btl (buf p2))) (Choice op1s) (f2 (BHD p2 buf))))
        else csingle (Read (Inr p2) (\<lambda>y. comp_op wire buf (Choice op1s) (f2 y)))) (cimage (\<lambda> op. comp_op wire buf op (Read p2 f2)) (cfilter (Not o finished) op1s)))"
 "comp_op wire buf (Choice op1s) (Write op2 q2 x2) =
    Choice (cinsert (Write (comp_op wire buf (Choice op1s) op2) (Inr q2) x2) (cimage (\<lambda> op. comp_op wire buf op (Write op2 q2 x2)) (cfilter (Not o finished) op1s)))"
   "comp_op wire buf (Choice op1s) (Choice op2s) =
    Choice (cUn (cimage (\<lambda> op. comp_op wire buf op (Choice op2s)) (cfilter (Not o finished) op1s))
        (cimage (\<lambda> op. comp_op wire buf (Choice op1s) op) (cfilter (Not o finished) op2s)))" 
...
lemma step_parallel_spin_op_False:
  "step op io op' \<Longrightarrow> op = comp_op (\<lambda>_. None) (\<lambda>_. []) \<otimes> \<otimes> \<Longrightarrow> False"
  apply (induct op io op' pred: step)
  apply (subst (asm) comp_op_code, simp)
  apply (subst (asm) comp_op_code, simp)
  apply (subst (asm) (2) comp_op_code)
  apply auto
  apply (subst (asm) (7) spin_op.code)
   apply simp
  apply force
  apply (subst (asm) (7) spin_op.code)
  apply force
...
lemma
  ...
  subgoal
    ...
    using step_parallel_spin_op_False apply auto
    done
  subgoal
    unfolding pcomp_op_def sim_def
    apply auto
...
lemma step_comp_op_L:
  ...
  subgoal for op ops l op' op2 buf

     apply simp
    apply (subst (1) comp_op_code)
    apply (rule step.intros(3))
       apply (rule cUnI1)
       apply (auto 1 1 simp add: Set.filter_def)
      apply (rule image_eqI[of _ _ op])
      apply (rule refl)
    apply (simp add: step_not_finished)
    apply auto
    done
  done
...
lemma step_comp_op_R:
  ...
  subgoal for op ops l op' op1 buf
    ...
    subgoal for p x
      ...
      apply (subst (1) comp_op_code)
        apply (clarsimp split: IO.splits option.splits intro: step.intros)
        using step.simps step_not_finished apply fastforce
        done
      done
    subgoal for p x
      apply hypsubst_thin
...
lemma comp_op_Some_Write_dummy_source_op_sink_op:
  "comp_op Some buf (Write (dummy_source_op 42 :: (1, 1, nat) op) 1 42) sink_op = comp_op Some buf' (dummy_source_op 42) sink_op"
  apply (coinduction arbitrary: buf buf' rule: op.coinduct_upto)
  subgoal for buf buf'
    apply auto
    subgoal premises prems
      ...
      subgoal for op
        ...
        subgoal
          ...
          apply (auto simp add: ranI split: if_splits)
          apply (smt (verit, del_insts) UNIV_I cUNIV.rep_eq comp_def dummy_source_op.cong_base dummy_source_op.cong_sym finished_WriteE image_iff member_filter sink_op.code)
          apply (smt (verit, del_insts) UNIV_I cUNIV.rep_eq comp_def dummy_source_op.cong_base dummy_source_op.cong_sym finished_WriteE image_iff member_filter sink_op.code)
          done
        subgoal
          apply (subst comp_op_code)
        ...
        subgoal
          ...
          apply (subst (2) sink_op.code)
          apply (auto simp add: ranI split: if_splits)
          apply (smt (verit, del_insts) UNIV_witness cUNIV.rep_eq comp_apply dummy_source_op.cong_base dummy_source_op.cong_sym dummy_source_op.cong_trans finished_WriteE imageI member_filter num1_eq1)          
          apply (smt (verit, del_insts) UNIV_witness cUNIV.rep_eq comp_apply dummy_source_op.cong_base dummy_source_op.cong_sym dummy_source_op.cong_trans finished_WriteE imageI member_filter num1_eq1)          
          done
        done
      subgoal for op
...
lemma step_dummy_source_op_sink_op_False:
  "step op io op' \<Longrightarrow> op = comp_op Some buf (dummy_source_op 42 :: (1, 1, nat) op) sink_op \<Longrightarrow> False"
  apply (induct op io op' arbitrary: buf pred: step)
    apply (subst (asm) comp_op_code, simp)
   apply (subst (asm) comp_op_code, simp)
  subgoal for op ops l op' buf
    apply (subst (asm) (2) comp_op_code)
    apply (auto split: if_splits)
     apply (subst (asm) (2) dummy_source_op.code)
     apply (auto split: if_splits)
    subgoal
      ...
      subgoal
        apply hypsubst_thin
        apply (subst (asm) (1) dummy_source_op.code)
        apply simp
        apply (subgoal_tac "op = comp_op Some buf (Write (dummy_source_op 42) 1 42) sink_op")
         defer
        subgoal
          by (smt (verit) image_iff member_filter num1_eq1)         
          apply simp
        using comp_op_Some_Write_dummy_source_op_sink_op apply fast
        done
      subgoal
        apply hypsubst_thin
        apply (subst (asm) (1) dummy_source_op.code)
...
lemma dummy_source_op_sink_op_sping_op:
  ...
  subgoal
    ...
    subgoal
      ...
      apply hypsubst_thin
      subgoal for io op'
        apply (drule step_dummy_source_op_sink_op_False)
         apply auto
        done
      done
    subgoal 
...
lemma step_bisim:
  ...
  apply (induct op io op' arbitrary: op'' pred: step)
  subgoal
  ...
  subgoal
    apply (erule bisim.cases)
    apply (force elim: simE)
    done
  done

lemma step_comp_op_silent_L:
  "step op1 io' op1' \<Longrightarrow>
   io' = Out p x \<Longrightarrow>
   wire p = Some q \<Longrightarrow>
   step (comp_op wire (BENQ q x buf) op1' op2) io op \<Longrightarrow>
   step (comp_op wire buf op1 op2) io op"
  apply (induct op1 io' op1' arbitrary: buf pred: step)
    apply simp_all
  subgoal for op' q buf
    apply (subst comp_op_code)
    apply simp
    apply (rule step.intros)
     apply (rule cinsertI1)
  ...
  subgoal premises prems for op'' ops l op' buf
    ...
    apply (rule step.intros(3)[rotated])
     ...
     apply assumption
    apply auto
    apply (metis (no_types, lifting) cfilter.rep_eq cimage_eqI cin.rep_eq comp_apply member_filter step_not_finished)
    done
  done

lemma Choice_with_spin_op_bisim:
  "Choice {|spin_op, op|} ~ op"
...
lemma cfilter_not_finshed_end_op[simp]:
  "cfilter (Not \<circ> finished) {|\<oslash>, Read 1 (\<lambda>_. \<oslash>)|} = {|Read 1 (\<lambda>_. \<oslash>)|}"
  by fastforce
...
lemma comp_op_cong_counter_example_1:
  "Choice {|end_op, Read (1::1) (\<lambda> _. end_op) |} \<bullet> Write W (1::1) 42 ~ Read (1::1) (\<lambda> _. end_op) \<bullet> Write W (1::1) 42"
  unfolding scomp_op_def
  apply auto
  apply (coinduction rule: bisim_coinduct_upto)
  apply (intro conjI)
...
lemma
  "Choice ops ~ op2 \<Longrightarrow>
   op |\<in>| ops \<Longrightarrow>
   \<not> finished op \<Longrightarrow>
   is_Read op2 \<and> op ~ op2 \<or> is_Write op2 \<and> op ~ op2 \<or> 
   is_Choice op2 \<and> op ~ Choice (cfilter (sim bisim op) (un_Choice op2))"
  apply (cases op2)
  subgoal for p f
    apply simp
    apply (coinduction arbitrary: op rule: bisim_coinduct_upto)
    ...
    apply hypsubst_thin
    unfolding sim_def
    apply (drule not_finished_stepes)
    apply safe
    subgoal
    apply (drule spec2)
    apply (drule mp)
  ...
  subgoal for op
    apply hypsubst_thin
      ...
      apply auto
      subgoal for x
        apply (subgoal_tac "\<exists> op'. step op (Inp p x) op'")
        defer
        subgoal
          by (metis bisim_sym finished_no_step step.intros(3) stepReadE step_bisim step_exchange)
      apply safe
      subgoal for op'
     apply (erule bisim.cases)
    apply hypsubst_thin
        ...
        apply (drule spec2)
        apply (drule mp)
         apply (rule step.intros(3)[of op _ "Inp p x"])
          apply assumption+
         apply (elim exE conjE)
        apply (intro exI conjI)
         apply assumption+
        apply (metis (no_types, lifting) IO.inject(1) bc_bisim bisim_sym op.distinct(1) op.distinct(3) op.inject(1) step.simps)
        done
      done
    done
  done
  ...
  subgoal for op
   apply hypsubst_thin
      unfolding sim_def
      apply auto
        apply (subgoal_tac "\<exists> op'. step op (Out p x) op'")
        defer
        subgoal
          by (metis bisim.cases finished_no_step simE step.intros(3) stepWriteE)
    apply safe
      subgoal for op'
     apply (erule bisim.cases)
    apply hypsubst_thin
        ...
        apply (drule spec2)
        apply (drule mp)
         apply (rule step.intros(3)[of op _ "Out p x"])
          apply assumption+
         apply (elim exE conjE)
        apply (intro exI conjI)
         apply assumption+
        using bc_sym bisim_cong_disj apply blast
        done
      done
    done
  subgoal for ops2
   ...
   apply (erule bisim.cases)
    ...
    apply auto
    subgoal for t' op''
      apply (intro conjI exI[of _ t'])
      apply (rule step.intros(3)[of  op''])
        apply simp
        apply safe
      oops
...
lemma sorried_1:
  "Choice ops ~ op2 \<Longrightarrow>
   op |\<in>| ops \<Longrightarrow>
   op ~ Choice (cfilter (sim bisim op) (choices op2))"
  oops



lemma
  "Choice {| Choice {| Write end_op (1::1) 1, Write end_op (1::1) 4|}, Choice {|Write end_op (1::1) (0::nat), Write end_op (1::1) 2|}|} ~ 
    Choice {| Choice {| Write end_op (1::1) 0, Write end_op (1::1) 4|}, Choice {|Write end_op (1::1) (1::nat), Write end_op (1::1) 2|}|}"
  apply (rule choices_Choice_bisim)
  apply auto
  done

lemma
  "Read (1::1) (\<lambda> _. end_op) \<bullet> Choice {| Choice {| Write end_op (1::1) 1, Write end_op (1::1) 4|}, Choice {|Write end_op (1::1) (0::nat), Write end_op (1::1) 2|}|} ~
   Read (1::1) (\<lambda> _. end_op) \<bullet> Choice {| Choice {| Write end_op (1::1) 0, Write end_op (1::1) 4|}, Choice {|Write end_op (1::1) (1::nat), Write end_op (1::1) 2|}|} \<Longrightarrow> False"
  unfolding scomp_op_def
  apply auto
  apply (erule bisim.cases)
  subgoal for s t
         ...
         apply (cases op1)
          subgoal for p f
            ...
            subgoal for x
              ...
              apply safe
              subgoal for op1''
              apply (intro conjI exI[of _ "comp_op wire buf op1'' op2'"])
                using step_comp_op_L apply fastforce
                apply (smt (verit, del_insts) bc_base)
                done
              done
            done
          subgoal for op1'' p x
            ...
            subgoal
              ...
              apply auto
                apply (intro conjI exI)
              using step_comp_op_L apply force
                apply (smt (verit, del_insts) bc_base)
              done
            subgoal for q
              apply (erule bisim.cases)
              ...
              apply hypsubst_thin
                subgoal for t' 
                  apply (intro conjI[rotated] exI[of _ t'])
                   apply assumption
                apply (rule step_comp_op_silent_L)
                    apply assumption+
                    apply auto
                  done
                done
...
lemma
  "step (map_op projl projr (comp_op Some (buf1 :: 'd \<Rightarrow> 'c buf) op1 (map_op projl projr (comp_op Some (buf2 :: 'e \<Rightarrow> 'c buf) op2 op3)))) io op \<Longrightarrow>
   ...
   op = map_op projl projr (comp_op Some buf1' op1' (map_op projl projr (comp_op Some buf2' op2' op3'))) \<and>
   step (map_op projl projr (comp_op Some buf2 (map_op projl projr (comp_op Some buf1 op1 op2)) op3)) io 
          (map_op projl projr (comp_op Some buf2' (map_op projl projr (comp_op Some buf1' op1' op2')) op3'))"
  apply (induct "map_op projl projr (comp_op Some (buf1 :: 'd \<Rightarrow> 'c buf) op1 (map_op projl projr (comp_op Some (buf2 :: 'e \<Rightarrow> 'c buf) op2 op3)))" io op arbitrary: op1 op2 op3 buf1 buf2 pred: step)
    apply (subst (asm) comp_op_code, simp)
   apply (subst (asm) comp_op_code, simp)
  subgoal for op ops l op' op1 op2 op3 buf1 buf2
    apply (subst (asm) (9) comp_op_code)
    apply auto
    subgoal for op''
      apply hypsubst_thin
      ...
      subgoal for op1' p x
        ...
        apply safe
        apply hypsubst_thin
        subgoal for op1'a op2' op3' buf1' buf2'
          apply (intro conjI exI)
           apply (rule refl)
          apply (subst (2) comp_op_code)
          apply simp
      ...
      subgoal for ops
        ...
        apply safe
        apply hypsubst_thin
        subgoal for op op1' op2' op3' buf1' buf2'
          apply (intro conjI exI)
           apply (rule refl)
          apply (subst (2) comp_op_code)
          apply simp
    ...
    subgoal for op''
      apply hypsubst_thin
      apply (subst (asm) (12) comp_op_code)
      apply auto
      subgoal for op'''
        apply hypsubst_thin
        apply (cases op2)
...
definition new_scomp_op (infixl "\<bullet>" 65) where
  "new_scomp_op op1 op2 = map_op projl projr (new_comp_op Some (\<lambda>_. []) op1 op2)"
...
lemma assoc_example:
  ...
  apply simp
  apply hypsubst_thin
  unfolding sim_def new_scomp_op_def
  apply (drule spec2)
  apply (drule mp)
   apply (rule step_map_op)
   apply (subst new_comp_op_code)
   apply (rule step.intros(3))
    apply (rule cUnI1)
    apply simp
    apply (rule disjI2)
   ...
   apply (rule step.intros(3))
    apply simp
    apply (rule disjI2)
    apply (subst (4) new_comp_op_code)
    apply simp
   apply (subst new_comp_op_code)
   apply simp
   apply (rule step.intros(3))
  ...
  subgoal
    ...
    apply (subst (asm) (2) new_comp_op_code)
    apply auto
    apply (subst (asm) (7) new_comp_op_code)
    apply auto
    subgoal
      apply (subst (asm) (4) new_comp_op_code)
      apply auto
...
lemma 
  ...
  subgoal for io op
    apply (subst (asm) (1) new_comp_op_code)
    apply auto
    apply (subst (asm) (3) new_comp_op_code)
    apply auto
    subgoal
      apply hypsubst_thin
      apply (intro exI conjI)
       apply (subst (2) new_comp_op_code)
       apply simp
       apply (subst (1) new_comp_op_code)
       apply simp
       apply (rule step.intros(3))
        apply simp
        apply (rule disjI1)
    ...
    subgoal
      ...
      subgoal for x
        apply (subst (asm) (5) new_comp_op_code)
        apply simp
        apply hypsubst_thin
        apply (subst (asm) (2) new_comp_op_code)
        apply auto
