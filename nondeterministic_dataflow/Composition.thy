...
lemma comp_op_code: "comp_op wire buf op1 op2 =
  ...
  (if diverged op1 \<and> diverged op2 then {|spin_op|} else if may_terminate op1 \<or> may_terminate op2 then {|End|} else {||}))"
...
lemma comp_op_simps[simp]:
  ...
  "comp_op wire buf (Read p1 f1) (Choice op2s) = 
    Choice (cUn (cinsert (Read (Inl p1) (\<lambda>y. comp_op wire buf (f1 y) (Choice op2s))) (cimage
       ...
       (sound_reads wire buf (cUnion (cimage choices op2s))))) (if can_end (Choice op2s) \<and> \<not> diverged (Choice op2s) then {|End|} else {||}))"
  ...
  "comp_op wire buf (Choice op1s) (Read p2 f2) =
    Choice (cUn (cUn (if p2 \<in> ran wire then (if BHD p2 buf = None then cempty else csingle (comp_op wire (buf(p2 := btl (buf p2))) (Choice op1s) (safe_read f2 (BHD p2 buf))))
       ...
       (cUnion (cimage choices op1s)))) (if can_end (Choice op1s) \<and> \<not> diverged (Choice op1s) then {|End|} else {||}))"
  ...
  "comp_op wire buf (Choice op1s) (Choice op2s) =
   ...
   (if diverged (Choice op1s) \<and> diverged (Choice op2s) then csingle spin_op 
   else (if may_terminate (Choice op1s) \<or> may_terminate (Choice op2s)  then {|End|} else {||})))"
          apply (subst comp_op_code, clarsimp simp add: image_iff)
  subgoal
    by (subst comp_op_code, auto simp add: image_iff)
  subgoal
    by (subst comp_op_code, auto simp add: image_iff Set.filter_def split: op.splits if_splits)
  subgoal
    by (subst comp_op_code, auto simp add: image_iff)
  subgoal
    by (subst comp_op_code, auto simp add: image_iff Set.filter_def split: op.splits if_splits)
...
lemma
  ...
  subgoal
    ...
    apply (auto 1 1 split: if_splits)
    subgoal
      apply (rule exI[of _ "map_op id (case_sum id id) (comp_op (\<lambda>_. None) (\<lambda>_. BEnded) (Write W 1 42) AW)"])
      apply safe
       apply (rule stepped.intros(3))
        apply (rule cinsertI2)+
        apply simp
        apply force
       apply (simp add: stepped.intros(2))
...
lemma
  ...
  subgoal
    ...
    apply (subst (asm) W.code)
    apply (subst (asm) comp_op_code)
    apply (force simp add: diverged.intros split: if_splits)
    done
  subgoal
    unfolding pcomp_op_def
    apply (subst (asm) spin_op.code)
  ...
  subgoal
    ...
    subgoal for l s'
      ...
      apply (intro conjI exI[of _ "comp_op (\<lambda>_. None) (\<lambda>_. BEnded) (Choice {|spin_op|}) W"] stepped.intros(3))
        apply force
       apply (rule stepped.intros(2))
      apply (metis (mono_tags, lifting) bc_base bc_sym cinsert_not_cempty csingleton_iff diverged.intros diverged_is_spin_op spin_op_diverged)
      done
    done
  done
...
lemma
  ...
  subgoal
    ...
    apply (simp add: diverged.intros Set.filter_def diverged.intros)
    done
  subgoal
...
lemma W_not_diverged[simp]:
  "\<not> diverged W"
  apply safe
  apply (subst (asm) W.code)
  apply (erule diverged.cases)
  apply auto
  done

lemma sink_op_not_diverged[simp]:
  "\<not> diverged sink_op"
  apply safe
...
lemma sink_op_can_end_op2[simp]:
  "can_end_op2 Some sink_op"
  apply coinduction
  apply (rule disjI2)
  apply (rule disjI1)
...
lemma can_end_W_sink_op[simp]:
  "can_end (comp_op Some buf W sink_op)"
  apply (coinduction arbitrary: buf)
  subgoal 
    unfolding scomp_op_def
    apply (rule disjI2)
    apply (subst (1) comp_op_code)
    apply (auto 1 1 simp add: ranI split: if_splits option.splits)
    subgoal
...
lemma sink_op_not_can_end[simp]:
  "\<not> can_end sink_op"
  apply (subst sink_op.code)
  apply auto
  done

lemma diverged_W_sink_op:
  "diverged (comp_op Some buf W sink_op)"
  apply (coinduction arbitrary: buf)
  subgoal 
    unfolding scomp_op_def
    apply (subst (1) comp_op_code)
...
lemma dierged_map_op[simp]:
  "diverged (map_op f g op) \<longleftrightarrow> diverged op"
  apply (rule iffI)
  subgoal
    ...
    subgoal for op
      apply (erule diverged.cases)
      apply auto
      apply (smt (verit, best) bot_cset.rep_eq can_end.simps can_end_map_op cimageE cimageI cin.rep_eq diverged_can_end empty_iff op.map(3) op.simps(3))
      done
    done
  subgoal
    apply (coinduction arbitrary: op)
...
lemma
  ...
  apply (coinduction rule: bisim_coinduct_upto)
  apply safe
       apply (simp_all add: diverged_W_sink_op)
  subgoal
    unfolding sim_def
    apply safe
    apply (meson dierged_map_op diverged_W_sink_op stepped_not_diverged)
    done
  subgoal
    unfolding sim_def
    apply safe
...
lemma diverged_AW_False:
  "diverged AW \<Longrightarrow> False"
  apply (subst (asm) AW.code)
  apply (erule diverged.cases)
  apply auto
  done
...
lemma scomp_op_AW_Id_can_end:
  "can_end (comp_op Some buf AW Id_op)"
  apply (coinduction arbitrary: buf)
  subgoal for buf
    apply (rule disjI2)
    apply (intro conjI exI disjI1)
...
lemma diverged_comp_op_Some_AW_Id_op_False:
  "diverged (comp_op Some (\<lambda>_. BEmpty) AW Id_op) \<Longrightarrow> False"
  apply (subst (asm) comp_op_code)
  apply (erule diverged.cases)
  apply (auto 1 1 split: if_splits)
  using diverged_AW_False apply blast+
  done


inductive comp_op_inv for wire io where
  "stepped op1 (Inp p x) op1' \<Longrightarrow> io = Inp (Inl p) x \<Longrightarrow> comp_op_inv wire io (comp_op wire buf op1' op2) buf op1 op2"
| "stepped op2 (Out p x) op2' \<Longrightarrow> io = Out (Inr p) x \<Longrightarrow> comp_op_inv wire io (comp_op wire buf op1 op2') buf op1 op2"
| "stepped op1 (Out p x) op1' \<Longrightarrow> p \<notin> dom wire \<Longrightarrow> io = Out (Inl p) x \<Longrightarrow> comp_op_inv wire io (comp_op wire buf op1' op2) buf op1 op2"
| "stepped op2 (Inp p x) op2' \<Longrightarrow> p \<notin> ran wire \<Longrightarrow> io = Inp (Inr p) x \<Longrightarrow> comp_op_inv wire io (comp_op wire buf op1 op2') buf op1 op2"
| "comp_op_inv wire io op (BENQ q x buf) op1' op2 \<Longrightarrow> stepped op1 (Out p x) op1' \<Longrightarrow> wire p = Some q \<Longrightarrow> comp_op_inv wire io op buf op1 op2"
| "comp_op_inv wire io op (BTL p buf) op1 op2' \<Longrightarrow> BHD p buf = Some x \<Longrightarrow> stepped op2 (Inp p x) op2' \<Longrightarrow> p \<in> ran wire \<Longrightarrow> comp_op_inv wire io op buf op1 op2"
  (* | "comp_op_inv wire io op (BTL p buf) op1 End \<Longrightarrow> BHD p buf = None \<Longrightarrow> p \<in> ran wire \<Longrightarrow> comp_op_inv wire io op buf op1 op2"
 *)

lemma stepped_comp_op_inv:
  "stepped (comp_op wire buf op1 op2) io op \<Longrightarrow>
   comp_op_inv wire io op buf op1 op2"
  ...
  subgoal for op ops l op' op1 op2 buf
    ...
    subgoal for p f ops
      ...
      subgoal
        ...
        subgoal
          by (metis Write_in_choices_stepped Read_in_choices_stepped cin.rep_eq comp_op_inv.intros stepped.simps)
        subgoal
          by (metis Write_in_choices_stepped Read_in_choices_stepped cin.rep_eq comp_op_inv.intros stepped.simps)
        subgoal
          by (metis Read_in_choices_stepped cin.rep_eq comp_op_inv.intros stepped.simps)
        subgoal
          by (metis Write_in_choices_stepped Read_in_choices_stepped cin.rep_eq comp_op_inv.intros stepped.simps)
        subgoal
          by (metis Write_in_choices_stepped Read_in_choices_stepped cin.rep_eq comp_op_inv.intros stepped.simps)
    ...
    subgoal
      apply hypsubst_thin
      apply (auto split: option.splits op.splits if_splits intro: comp_op_inv.intros intro!: stepped.intros; hypsubst_thin)
      done 
    subgoal
      apply hypsubst_thin
      apply (auto split: option.splits op.splits if_splits intro: comp_op_inv.intros intro!: stepped.intros; hypsubst_thin)
    ...
    subgoal
      apply hypsubst_thin
      apply (clarsimp split: option.splits op.splits if_splits intro: comp_op_inv.intros intro!: stepped.intros; hypsubst_thin)
      subgoal
        apply (auto 1 1 split: option.splits op.splits if_splits intro: comp_op_inv.intros intro!: stepped.intros; hypsubst_thin)
        subgoal
          by (metis Write_in_choices_stepped Read_in_choices_stepped cin.rep_eq comp_op_inv.intros stepped.simps)
        subgoal
          by (metis Write_in_choices_stepped Read_in_choices_stepped cin.rep_eq comp_op_inv.intros stepped.simps)
      ...
      subgoal
        apply (auto 1 1 split: option.splits op.splits if_splits)
        subgoal
          by (metis Write_in_choices_stepped Read_in_choices_stepped cin.rep_eq comp_op_inv.intros stepped.simps)
        subgoal
          by (metis Write_in_choices_stepped Read_in_choices_stepped cin.rep_eq comp_op_inv.intros stepped.simps)
    ...
    subgoal
      ...
      subgoal
        apply (auto 1 1 split: option.splits op.splits if_splits intro: comp_op_inv.intros intro!: stepped.intros; hypsubst_thin)
        subgoal
          by (metis Write_in_choices_stepped domIff Read_in_choices_stepped cin.rep_eq comp_op_inv.intros stepped.simps)
        subgoal
          by (metis Write_in_choices_stepped domIff Read_in_choices_stepped cin.rep_eq comp_op_inv.intros stepped.simps)
        subgoal
          by (metis Write_in_choices_stepped domIff Read_in_choices_stepped cin.rep_eq comp_op_inv.intros stepped.simps)
      ...
      subgoal
        by (metis Write_in_choices_stepped domIff Read_in_choices_stepped cin.rep_eq comp_op_inv.intros stepped.simps)
      subgoal
        by (auto 1 1 split: option.splits op.splits if_splits intro: comp_op_inv.intros intro!: stepped.intros; hypsubst_thin)
      subgoal
        apply (auto 1 1 split: option.splits op.splits if_splits intro: comp_op_inv.intros intro!: stepped.intros; hypsubst_thin)
        subgoal
          by (metis Write_in_choices_stepped domIff Read_in_choices_stepped cin.rep_eq comp_op_inv.intros stepped.simps)
    ...
    subgoal
      ...
      apply (clarsimp split: option.splits op.splits if_splits intro: comp_op_inv.intros intro!: stepped.intros; hypsubst_thin)
      subgoal
        apply (auto 0 0 split: option.splits op.splits if_splits intro: comp_op_inv.intros intro!: stepped.intros; hypsubst_thin)
        subgoal
          by (metis Write_in_choices_stepped domIff Read_in_choices_stepped cin.rep_eq comp_op_inv.intros stepped.simps)
        subgoal
          by (metis Write_in_choices_stepped domIff Read_in_choices_stepped cin.rep_eq comp_op_inv.intros stepped.simps)
        ...
        subgoal
          by (metis Write_in_choices_stepped domIff Read_in_choices_stepped cin.rep_eq comp_op_inv.intros stepped.simps)
        subgoal
          using stepped_spin_op_no_label by blast
        done
      subgoal
        apply (auto 0 0 split: option.splits op.splits if_splits intro: comp_op_inv.intros intro!: stepped.intros dest: stepped_spin_op_no_label; hypsubst_thin)
                            apply (metis Write_in_choices_stepped domIff Read_in_choices_stepped cin.rep_eq comp_op_inv.intros stepped.simps)+
        done
      subgoal
        apply safe
        subgoal
          apply (auto split: option.splits op.splits if_splits intro: comp_op_inv.intros intro!: stepped.intros dest: stepped_spin_op_no_label; hypsubst_thin)
            apply (metis Write_in_choices_stepped domIff Read_in_choices_stepped cin.rep_eq comp_op_inv.intros stepped.simps)+
          done
        subgoal
          apply (auto 1 1 split: option.splits op.splits if_splits intro: comp_op_inv.intros intro!: stepped.intros dest: stepped_spin_op_no_label; hypsubst_thin)
            apply (metis Write_in_choices_stepped domIff Read_in_choices_stepped cin.rep_eq comp_op_inv.intros stepped.simps)+
          done
        subgoal
          apply (auto 1 1 split: option.splits op.splits if_splits intro: comp_op_inv.intros intro!: stepped.intros dest: stepped_spin_op_no_label; hypsubst_thin)
        ...
        subgoal for x
          ...
          subgoal
            ...
            subgoal
              by (metis Read_in_choices_stepped cin.rep_eq comp_op_inv.intros(6) option.simps(5) stepped.intros(3))
            subgoal
              by (smt (verit) Read_in_choices_stepped cin.rep_eq comp_op_inv.intros(4) stepped.simps steppedReadE)
            done
          subgoal
            apply (simp split: if_splits)
            by (metis Write_in_choices_stepped cin.rep_eq comp_op_inv.intros(2) stepped.intros(3) steppedWriteE)
          subgoal
            apply (auto split: if_splits)
            done
          done
...
lemma comp_op_inv_Sone_AW_Write:
  "comp_op_inv Some io op buf op1 op2 \<Longrightarrow>
   op1 = AW \<Longrightarrow>
   op2 = Write Id_op p x \<Longrightarrow>
   io = Out (Inr p) x \<and> (\<exists> buf'. op = comp_op Some buf' op1 Id_op)"
  apply (induct op buf op1 op2 rule: comp_op_inv.induct)
  subgoal
    using stepped_AW_inv by blast
  subgoal
    by fastforce
  subgoal
    by blast
  ...
  subgoal
    apply (auto simp add: ranI split: if_splits)
     apply (frule stepped_AW_inv)
      apply auto
    using stepped_AW_inv apply blast
    done
  subgoal
    apply (auto simp add: ranI split: if_splits)
    done
...
lemma stepped_comp_op_Some_AW_Id_op:
  "stepped (comp_op Some buf op1 op2) io op \<Longrightarrow>
   op1 = AW \<Longrightarrow>
   op2 = Id_op \<or> op2 = Write Id_op 1 42 \<Longrightarrow>
   (\<forall> x. x \<in> buf.set_buf (buf 1) \<longrightarrow> x = 42) \<Longrightarrow>
   io = Out (Inr 1) 42 \<and> (\<exists> buf'. op' = comp_op Some buf' op1 Id_op)"
  ...
  subgoal 
    ...
    subgoal
      apply auto
        apply (metis IO.simps(4) Id_op.code steppedReadE)
       apply (metis IO.distinct(1) Id_op.code steppedReadE)
      oops
        (*
    subgoal
      by blast
    ...
    subgoal for op p buf op1 op2' x op2
      ...
      apply hypsubst_thin
      apply (frule stepped_Id_op_inv)
      apply simp
       apply auto
      apply (metis bhd.elims btl.simps(2) num1_eq1 observation.simps(3) option.discI option.inject)
      apply (drule comp_op_inv_Sone_AW_Write[where p=1])
        apply auto
      apply (metis bhd.elims bhd.simps(2) buf.set_intros(1) observation.distinct(1) observation.simps(1) option.inject option.simps(3))
      done
    done
  done

lemma
  "bisim (map_op projl projr (comp_op Some (\<lambda>_. BEmpty) AW Id_op)) AW"
  apply (coinduction rule: bisim_coinduct_upto)
  subgoal
    apply (auto simp add: diverged_AW_False scomp_op_AW_Id_can_end dest: diverged_AW_False diverged_comp_op_Some_AW_Id_op_False)
    subgoal
      unfolding sim_def
      apply safe
      oops*)
...
coinductive comp_op_can_end for wire where
  "may_terminate op1 \<or> may_terminate op2 \<Longrightarrow> \<not> diverged op1 \<Longrightarrow> \<not> diverged op2 \<Longrightarrow> comp_op_can_end wire buf op1 op2"
| "diverged op1 \<Longrightarrow> diverged op2 \<Longrightarrow> comp_op_can_end wire buf op1 op2"
...
lemma can_end_id_op[simp]:
  "can_end (id_op buf)"
  apply (subst id_op_code)
  apply auto
  done

lemma not_diverged[simp]:
  "\<not> diverged (id_op buf)"
  apply (subst id_op_code)
  apply auto
  done

lemma stepped_id_op_Inp_Observed:
  "stepped (id_op buf) io op1' \<Longrightarrow>
   io = Inp p (Observed x) \<Longrightarrow>
   op1' = id_op (BENQ p x buf)"
  apply (induct "id_op buf" io op1' arbitrary: buf rule: stepped.induct)
    apply simp_all
...
lemma stepped_id_op_Inp_EOS:
  "stepped (id_op buf) io op1' \<Longrightarrow>
   io = Inp p EOS \<Longrightarrow>
   op1' = id_op buf"
  apply (induct "id_op buf" io op1' arbitrary: buf rule: stepped.induct)
    apply simp_all
   apply (subst (asm) id_op_code)
...
lemma stepped_id_op_Out:
  "stepped (id_op buf) io op1' \<Longrightarrow>
   io = Out p x \<Longrightarrow>
   op1' = id_op (BTL p buf) \<and> BHD p buf = Some (Observed x)"
  apply (induct "id_op buf" io op1' arbitrary: buf rule: stepped.induct)
    apply simp_all
   apply (subst (asm) id_op_code)
   apply simp
...
lemma stepped_comp_op_Some_id_op_id_op:
  "stepped (comp_op Some buf2 op1 op2) io op \<Longrightarrow>
   ...
   (\<exists> p x. io = Inp (Inl p) (Observed x) \<and>
      (\<exists> buf1' buf2' buf3'. op = comp_op Some buf2' (id_op (BENQ p x buf1')) (id_op buf3') \<and> buf1 >> buf2 >> buf3 = buf1' >> buf2' >> buf3')) \<or>
   (\<exists> p x. io = Inp (Inl p) EOS \<and> (\<exists> buf1' buf2' buf3'. op = comp_op Some buf2' (id_op buf1') (id_op buf3') \<and> buf1 >> buf2 >> buf3 = buf1' >> buf2' >> buf3')) \<or>
   (\<exists> p x. io = Out (Inr p) x \<and>
      (\<exists> buf1' buf2' buf3'. op = comp_op Some buf2' (id_op buf1') (id_op (BTL p buf3')) \<and> BHD p buf3' = Some (Observed x) \<and> buf1 >> buf2 >> buf3 = buf1' >> buf2' >> buf3'))"
  apply (drule stepped_comp_op_inv)
  subgoal
    apply (rotate_tac 2)
    apply (induct op buf2 op1 op2 arbitrary: buf1 buf3 rule: comp_op_inv.induct)
...
lemma BHD_BAPPEND[simp]:
  ...
  by (metis BAPPEND_bappend)

lemma bhd_Some_bappend:
  "bhd ys = Some (Observed x) \<Longrightarrow>
   bhd (bappend xs ys) = Some (Observed x)"
  apply (induct xs arbitrary: ys)
    apply auto[2]
  apply (smt (verit, best) bappend.simps(3) benq.simps(3) bhd.elims bhd.simps(3) observation.simps(3) option.distinct(1) option.inject)
  done

lemma BHD_Some_BAPPEND:
  "BHD p buf2 = Some (Observed x) \<Longrightarrow>
  ...
  subgoal for buf1 buf2 buf3
    ...
    subgoal
      ...
      subgoal for io op
        ...
        apply safe
        apply (drule stepped_comp_op_Some_id_op_id_op)
          apply simp_all
        ...
        subgoal for io' op'' p x
          ...
          subgoal
            ...
            subgoal for q
              apply (cases "p = q")
              subgoal
                by (metis BAPPEND_bappend bappend.simps(1) benq_bappend bhd.elims fun_upd_same option.simps(3))
              subgoal
          ...
          subgoal
            ...
            subgoal for q
              apply (cases "p = q")
              subgoal
                by (smt (verit) BAPPEND.elims BAPPEND_bappend benq_bappend fun_upd_same observation.case_eq_if option.simps(5))
              subgoal
                by (metis BAPPEND_bappend fun_upd_other)
              done
            done
        ...
        subgoal for io' op'' p x
          ...
          apply hypsubst_thin
          subgoal for buf1' buf2' buf3'
            apply (rule exI[of _ "id_op
         (\<lambda>q. if p = q
              then btl (case BHD p buf1' of None \<Rightarrow> (buf2' >> buf3') p
                        | Some (Observed xa) \<Rightarrow> (buf1'(p := btl (buf1' p)) >> (buf2' >> buf3')(p := benq xa ((buf2' >> buf3') p))) p | Some EOS \<Rightarrow> (buf2' >> buf3') p)
              else bappend (buf1' q) ((buf2' >> buf3') q))"])
            apply (intro conjI exI)
             apply (subst id_op_code)
             apply (rule stepped.intros(3))
              apply simp
              ...
              apply (rule image_eqI[of _ _ p])
               apply (rule refl)
            subgoal
              apply (auto simp add: cUNIV.rep_eq Set.filter_def)
              apply (rule exI[of _ x])
              apply (auto split: option.splits)
              using BHD_Some_BAPPEND
                apply (smt (verit, ccfv_threshold) BAPPEND_BTL_Some_BENQ bhd.elims observation.case_eq_if observation.sel option.discI option.sel)
              using BHD_Some_BAPPEND
               apply (smt (verit, ccfv_threshold) BAPPEND_BTL_Some_BENQ bhd.elims observation.case_eq_if observation.sel option.discI option.sel)
              using BHD_Some_BAPPEND
              apply (smt (verit, ccfv_threshold) BAPPEND_BTL_Some_BENQ bhd.elims observation.case_eq_if observation.sel option.discI option.sel)
            ...
            subgoal
              by (smt (verit) BAPPEND_BTL_Some_BENQ BHD_Some_BAPPEND bhd.elims obs_def observation.simps(4) observation.simps(5) option.case_eq_if option.sel stepped.simps)
            subgoal
              apply (rule bc_base)
              apply (intro conjI exI)
               apply (rule refl)
...
lemma stepped_pcomp_op_inv:
  "stepped (pcomp_op op1 op2) io op \<Longrightarrow>
   (\<exists> op1' p x op2'. stepped op1 (Inp p x) op1' \<and> op = (pcomp_op op1' op2) \<and> io = Inp (Inl p) x) \<or>
   (\<exists> op1' p x op2'. stepped op1 (Out p x) op1' \<and> op = (pcomp_op op1' op2) \<and> io = Out (Inl p) x) \<or>
   (\<exists> op2' p x op1'. stepped op2 (Inp p x) op2' \<and> op = (pcomp_op op1 op2') \<and> io = Inp (Inr p) x) \<or>
   (\<exists> op2' p x op1'. stepped op2 (Out p x) op2' \<and> op = (pcomp_op op1 op2') \<and> io = Out (Inr p) x)"
  apply (induct "pcomp_op op1 op2" io op arbitrary: op1 op2 rule: stepped.induct)
...
lemma stepped_pcomp_op_L:
  "stepped op1 io op1' \<Longrightarrow>
   stepped (pcomp_op op1 op2) (map_IO Inl Inl id io) (pcomp_op op1' op2)"
  apply (induct op1 io op1' arbitrary: op2 rule: stepped.induct)
  unfolding pcomp_op_def
  subgoal
    apply (subst (1) comp_op_code)
    apply (rule stepped.intros(3))
...
lemma stepped_pcomp_op_R:
  "stepped op2 io op2' \<Longrightarrow>
   stepped (pcomp_op op1 op2) (map_IO Inr Inr id io) (pcomp_op op1 op2')"
  apply (induct op2 io op2' arbitrary: op1 rule: stepped.induct)
  unfolding pcomp_op_def
  subgoal
    apply (subst (1) comp_op_code)
    apply (rule stepped.intros(3))
  ...
  subgoal for op1 op2
    ...
    subgoal for ops1 ops2
      ...
      subgoal 
        ...
        apply (metis cin.rep_eq diverged.simps op.inject(3) stepped_not_diverged)
        apply (metis cin.rep_eq diverged.simps op.inject(3) stepped_not_diverged)
        subgoal for op2' p x op
          apply (intro conjI exI)
          apply (rule stepped.intros(3)[rotated])
          apply (drule stepped_map_op[where f=Inr and g=Inr])
      ...
      subgoal 
        ...
        subgoal for l s' io' op''
          ...
          apply (rule bc_base)
          apply blast
          apply (rule stepped_pcomp_op_R[unfolded pcomp_op_def])
          apply assumption
          done
        done
      done
...
lemma stepped_map_op_reassoc_map_IO_assoc:
  "stepped op (map_IO assoc assoc id io) op' \<Longrightarrow>
   stepped (map_op reassoc reassoc op) io (map_op reassoc reassoc op')"
  apply (induct op "map_IO assoc assoc id io" op' arbitrary: io rule: stepped.induct)
  subgoal for p f x io
    apply (cases io)
    apply (simp_all add: observation.map_id)
    apply (metis comp_apply id_apply reassoc_assoc stepped.simps)
    done
  subgoal for p f x io
    apply (cases io)
    apply (simp_all add: observation.map_id)
  ...
  subgoal for op ops op' io
    ...
    apply (simp_all add: observation.map_id)
    subgoal
      by (metis IO.simps(9) cimage_eqI cin.rep_eq observation.map_id stepped.intros(3))
    subgoal
      by (metis IO.simps(10) cimageI cin.rep_eq id_def stepped.simps)
    done
  done

lemma stepped_map_op_assoc_map_IO_reassoc:
  "stepped op (map_IO reassoc reassoc id io) op' \<Longrightarrow>
   stepped (map_op assoc assoc op) io (map_op assoc assoc op')"
  apply (induct op "map_IO reassoc reassoc id io" op' arbitrary: io rule: stepped.induct)
  subgoal for p f x io
    apply (cases io)
    apply (simp_all add: observation.map_id)
    apply (smt (verit) assoc.simps(1) assoc.simps(2) assoc.simps(3) comp_apply reassoc.elims stepped.intros(1))
    done
  subgoal for p f x io
    apply (cases io)
    apply (simp_all add: observation.map_id)
    apply (smt (verit, ccfv_SIG) assoc.simps(1) assoc.simps(2) assoc.simps(3) reassoc.elims stepped.intros(2))
    done
  subgoal for op ops op' io
    apply (cases io)
    apply (simp_all add: observation.map_id)
...
lemma stepped_map_op_reassocD:
  "stepped (map_op reassoc reassoc op) io op' \<Longrightarrow>
   stepped op (map_IO assoc assoc id io) (map_op assoc assoc op')"
  apply (induct "map_op reassoc reassoc op" io op' arbitrary: op rule: stepped.induct)
  subgoal
    apply (simp_all add: observation.map_id)
    apply (smt (verit, del_insts) assoc_reassoc comp_apply observation.map_ident op.map_comp op.map_id op.simps(25) stepped.intros(1))
    done
  subgoal for op q x opa
    apply (cases op)
    apply (simp_all add: observation.map_id)
    apply (metis (no_types, lifting) assoc_reassoc id_apply op.map_comp op.map_id0 op.simps(25) op.simps(26) stepped.intros(2))
    apply (metis (no_types, opaque_lifting) assoc_reassoc op.map_comp op.map_id op.simps(26) stepped.intros(2))
    apply (metis (no_types, opaque_lifting) assoc_reassoc id_apply op.map_comp op.map_id0 op.simps(26) op.simps(27) stepped.intros(2))
    done
  subgoal for op ops l op' op''
    apply (cases op)
    apply (simp_all add: observation.map_id)
    subgoal
      apply (auto 10 10 simp add: sup_cset.rep_eq cinsert.rep_eq cimage.rep_eq bot_cset.rep_eq intro: stepped.intros; hypsubst_thin?)
      apply (smt (verit, del_insts) IO.simps(9) assoc_reassoc cin.rep_eq observation.map_id op.inject(1) op.map_comp op.map_id op.simps(25) reassoc_assoc stepped.intros(1) stepped.intros(3) stepped_map_op_assoc_map_IO_reassoc)
      done
    subgoal
      apply (auto simp add: sup_cset.rep_eq cinsert.rep_eq cimage.rep_eq bot_cset.rep_eq intro: stepped.intros; hypsubst_thin?)
      apply (smt (verit, ccfv_threshold) assoc_reassoc cimageI cin.rep_eq id_apply op.map_comp op.map_id0 op.simps(26) op.simps(27) stepped.intros(2) stepped.intros(3))
      done
    subgoal
      apply (cases op'')
      apply (auto simp add: sup_cset.rep_eq cinsert.rep_eq cimage.rep_eq bot_cset.rep_eq intro: stepped.intros; hypsubst_thin?)+
...
lemma stepped_map_op_assocD:
  "stepped (map_op assoc assoc op) io op' \<Longrightarrow>
   stepped op (map_IO reassoc reassoc id io) (map_op reassoc reassoc op')"
  apply (induct "map_op assoc assoc op" io op' arbitrary: op rule: stepped.induct)
  subgoal
    apply (simp_all add: observation.map_id)
    apply (smt (verit, del_insts) reassoc_assoc comp_apply observation.map_ident op.map_comp op.map_id op.simps(25) stepped.intros(1))
    done
  subgoal for op q x opa
    apply (cases op)
    apply (simp_all add: observation.map_id)
    apply (metis (no_types, lifting) reassoc_assoc id_apply op.map_comp op.map_id0 op.simps(25) op.simps(26) stepped.intros(2))
    apply (metis (no_types, opaque_lifting) reassoc_assoc op.map_comp op.map_id op.simps(26) stepped.intros(2))
    apply (metis (no_types, opaque_lifting) reassoc_assoc id_apply op.map_comp op.map_id0 op.simps(26) op.simps(27) stepped.intros(2))
    done
  subgoal for op ops l op' op''
    apply (cases op)
    apply (simp_all add: observation.map_id)
    subgoal
      apply (auto 10 10 simp add: sup_cset.rep_eq cinsert.rep_eq cimage.rep_eq bot_cset.rep_eq intro: stepped.intros; hypsubst_thin?)
      apply (smt (verit, del_insts) IO.simps(9) assoc.simps(1) assoc.simps(2) assoc.simps(3) cin.rep_eq observation.map_id op.map_comp op.map_id reassoc.elims reassoc_assoc reassoc_assoc stepped.intros(1) stepped.intros(3) stepped_map_op_reassoc_map_IO_assoc)
      done
    subgoal
      apply (auto simp add: sup_cset.rep_eq cinsert.rep_eq cimage.rep_eq bot_cset.rep_eq intro: stepped.intros; hypsubst_thin?)
      apply (smt (verit, del_insts) cimageI cin.rep_eq id_def op.map_comp op.map_id0 op.simps(26) op.simps(27) reassoc_assoc stepped.intros(2) stepped.intros(3))
      done
    subgoal
      apply (cases op'')
      apply (auto simp add: sup_cset.rep_eq cinsert.rep_eq cimage.rep_eq bot_cset.rep_eq intro: stepped.intros; hypsubst_thin?)+
...
lemma stepped_pcomp_assoc_reassoc_Inl:
  "stepped op (Inp (Inl (Inl p)) x) (map_op assoc assoc op') \<Longrightarrow>
   stepped (map_op reassoc reassoc op) (Inp (Inl p) x) op'"
  by (metis (no_types, lifting) IO.simps(9) assoc.simps(1) observation.map_id op.map_comp op.map_id reassoc_assoc stepped_map_op_reassoc_map_IO_assoc)

lemma stepped_pcomp_reassoc_assoc_Inl:
  "stepped op (Inp (Inl p) x) (map_op reassoc reassoc op') \<Longrightarrow>
   stepped (map_op assoc assoc op) (Inp (Inl (Inl p)) x) op'"
  by (metis (no_types, lifting) IO.simps(9) assoc_reassoc observation.map_id op.map_comp op.map_id reassoc.simps(1) stepped_map_op_assoc_map_IO_reassoc)
...
lemma bisim_assoc_reassoc:
  ...
  subgoal for op op'
    ...
    subgoal for s t
      ...
      apply hypsubst_thin
      subgoal for l s
        by (smt (verit, del_insts) assoc_reassoc bc_base bisim_sym op.map_comp op.map_id reassoc_assoc stepped_map_op_reassocD stepped_map_op_reassoc_map_IO_assoc)
      subgoal for l s
        apply hypsubst_thin
        apply (drule spec2)
        apply (drule mp)
        apply (rule stepped_map_op_reassocD[of _ l s])
        ...
        apply assumption
        apply auto
        apply (metis (mono_tags, lifting) bc_base bc_sym stepped_map_op_reassoc_map_IO_assoc)
        done
      subgoal for l s
        apply hypsubst_thin
        apply rotate_tac
        ...
        apply assumption
        apply auto
        apply (smt (verit, del_insts) bc_base bc_sym op.map_comp op.map_id reassoc_assoc stepped_map_op_reassoc_map_IO_assoc)
        done
      subgoal for l s
        apply hypsubst_thin
        apply (drule spec2)
      ...
      subgoal for l s'              
        ...
        subgoal for op1' p x
          ...
          subgoal for op2'
            ...
            apply (intro conjI)
            subgoal
              using stepped_map_op_reassoc_map_IO_assoc stepped_pcomp_op_L
          ...
          subgoal for op23'
            ...
            subgoal for op2' pa xa
              ...
              subgoal for op2''
                ...
                apply (intro conjI)
                subgoal
                  apply (drule stepped_pcomp_op_R[of _ _ _ op1])
                  apply (drule stepped_pcomp_op_L[of _ _ _ op3])
                  apply (rule stepped_map_op_reassoc_map_IO_assoc)
                  apply (simp add: observation.map_id)
                  done
                subgoal
                  by (auto intro: bc_base)
            ...
            subgoal for op3' pa xa
              ...
              subgoal for op2''
                ...
                apply (intro conjI)
                subgoal
                  apply (drule stepped_pcomp_op_R[of _ _ _ "pcomp_op op1 op2"])
                  apply (rule stepped_map_op_reassoc_map_IO_assoc)
                  apply (simp add: observation.map_id)
                  done
                subgoal
      ...
      subgoal for l s'    
        ...
        subgoal for io' op'' op3' p x
          ...
          subgoal for op12
            ...
            apply (intro conjI)
            subgoal
              apply (drule stepped_pcomp_op_R[of _ _ _ op2])
              apply (drule stepped_pcomp_op_R[of _ _ _ op1])
              apply (simp add: observation.map_id)
              done
            subgoal
...
lemma can_end_scomp_op_Id_op_False:
  "can_end (scomp_op Id_op op) \<Longrightarrow> False"
  ...
  subgoal
    ...
    apply (erule stepped_choicesE)
    apply auto  
    apply (smt (verit) IO.distinct(1) can_end_op1_ReadE comp_op_can_end.simps steppedReadE stepped_End)
    done
  done
...
lemma comp_op_inv_Some_AW:
  ...
  subgoal
    by (auto dest: stepped_AW_inv)
  subgoal
    by (auto dest: stepped_AW_inv)
  subgoal
    by (auto dest: stepped_AW_inv)
  subgoal
    by (auto dest: stepped_AW_inv stepped_W_inv)
  subgoal
    by (auto dest: stepped_AW_inv stepped_W_inv)
  subgoal
    apply simp
...
lemma
  ...
  subgoal
    ...
    subgoal for io op'
      apply (drule stepped_map_op_inv)
      apply clarsimp
      apply (frule stepped_comp_op_inv)
      apply hypsubst_thin
      subgoal for io op
        apply (frule comp_op_inv_Some_AW)
        apply simp_all
        apply (rule exI[of _ AW])
        apply auto
        subgoal
...
lemma 
  "traced End ios \<Longrightarrow> traced (scomp_op Id_op (Choice {|Read 1 (\<lambda>_. End), End|})) ios \<Longrightarrow> False"
  unfolding scomp_op_def
  apply (subst (asm) Id_op.code)
