...
lemma wms_empty[simp]:
  ...
  by (induction xs) (auto intro: list.set_intros simp add: is_Data_def)
...
next
  case (Cons a xs)
  then show ?case
    by (metis event.exhaust insertE insertI1 insertI2 list.set_intros(1,2) set_ConsD vimageD vimageI2 wms.simps(2,3))
qed

lemma set_map_filter_case_event_Watermark[simp]:
  "set (List.map_filter (case_event (\<lambda>_. Map.empty) Some) xs) = Watermark -` (set xs)"
  by (induction xs) (auto simp add: map_filter_simps split: event.splits)
...
lemma monotone_LCons_Watermark[simp]:
  "monotone (LCons (Watermark wm) lxs) WM \<longleftrightarrow> monotone lxs (insert wm WM)"
  using monotone.cases LConsR by fastforce

lemma monotone_LCons_Data:
  "monotone (LCons (Data t d) lxs) WM \<Longrightarrow> monotone lxs WM"
...
lemma strict_monotone_good_example:
  \<open>monotone good_example {}\<close>
proof -
  have \<open>monotone (LNil :: (nat, String.literal) event llist) ({14,3})\<close>
    by (rule LNil)
  hence \<open>monotone (LCons (Watermark 14) (LNil :: (nat, String.literal) event llist)) {3}\<close>
    using LConsR by blast
  hence \<open>monotone (LCons (Data 4 STR ''data'') (LCons (Watermark 14) (LNil :: (nat, String.literal) event llist))) {3}\<close>
    using LConsL[where ?WM=\<open>{3}\<close>] by force
  hence \<open>monotone (LCons (Watermark 3) (LCons (Data 4 STR ''data'') (LCons (Watermark 14) (LNil :: (nat, String.literal) event llist)))) {}\<close>
    using LConsR by blast
  thus ?thesis
    using LConsL by fast
qed

lemma Data_tail_ahead_of_t:
  \<open>x \<in> lset lxs \<Longrightarrow> x = Data t d \<Longrightarrow> monotone (LCons (Watermark wm) lxs) WM \<Longrightarrow> \<not> t \<le> wm\<close>
proof (induction lxs arbitrary: WM rule: Coinductive_List.lset_induct)
  case (find xs)
  then show ?case
    by (meson LConsData LConsWatermark insert_iff)
next
  case (step x' xs)
  then show ?case
    by (metis (no_types, opaque_lifting) LConsData event.exhaust insert_commute monotone_LCons_Watermark)
qed

lemma Data_set_strict_monotone_not_GE:
  \<open>Data t d \<in> lset xs \<Longrightarrow> monotone xs WM \<Longrightarrow> \<forall>wm \<in> WM. \<not> t \<le> wm\<close>
proof (induction xs arbitrary: WM rule: Coinductive_List.lset_induct)
  case (find xs)
  then show ?case
    by (meson LConsData)
next
...
lemma strict_monotone_remove_wm[intro]:
  \<open>monotone xs (insert wm WM) \<Longrightarrow> monotone xs WM\<close>
proof (coinduction arbitrary: xs WM rule: monotone.coinduct)
  case monotone
  then show ?case
    using monotone.cases by (smt (verit, best) insertCI insert_commute)
qed

lemma strict_monotone_drop_head[intro]:
  \<open>monotone (LCons x xs) WM \<Longrightarrow> monotone xs WM\<close>
  by (metis event.exhaust monotone_LCons_Data monotone_LCons_Watermark strict_monotone_remove_wm)
...
lemma strict_monotone_ldrop_aux:
  \<open>monotone lxs wm \<Longrightarrow> ldrop (enat n) lxs = lxs' \<Longrightarrow> monotone lxs' wm\<close>
proof (induction n arbitrary: lxs)
  case 0
  then show ?case
    by (simp add: enat_0)
next
...
lemma strict_monotone_ldrop:
  \<open>monotone lxs wm \<Longrightarrow> ldrop n lxs = lxs' \<Longrightarrow> monotone lxs' wm\<close>
  by (cases n) (simp_all add: strict_monotone_ldrop_aux monotone.LNil)

lemma strict_monotone_LCons_Watermark_insert:
  "monotone (LCons (Watermark wm) lxs) WM \<Longrightarrow> monotone lxs (insert wm WM)"
  by (erule monotone.cases) simp_all

lemma ldropn_lfinite_lfilter:
  \<open>n < llength lxs \<Longrightarrow> \<forall>x \<in> lset (ldropn n lxs). \<not> P x \<Longrightarrow> lfinite (lfilter P lxs)\<close>
proof (induction n arbitrary: lxs)
  case 0
  then show ?case
    by simp
next
  case (Suc n)
  then show ?case
    by (cases lxs) (simp_all add: Suc_ile_eq)
qed
...
lemma strict_monotone_lfinite_lfilter_le_t_alt:
  ...
  subgoal
    ...
    subgoal for n
      apply (rule ldropn_lfinite_lfilter[where n=n])
       apply simp
      apply safe
      subgoal for x
...
lemma strict_monotone_LCons_Watermark_Data_not_ge:
  \<open>monotone (LCons (Watermark wm) lxs) WM \<Longrightarrow> Data t d \<in> lset lxs \<Longrightarrow> \<not> t \<le> wm\<close>
  by (erule monotone.cases) (auto simp add: Data_set_strict_monotone_not_GE)
...
lemma strict_monotone_lfinite_lfilter_le_t:
  ...
  using lfinite_lfilterI apply blast
  subgoal
    apply (rule ldropn_lfinite_lfilter[where n="Suc (stop_gathering_location t lxs)"])
...
lemma productive_drop_head: "productive (LCons a xs) \<Longrightarrow> productive xs"
  by (auto elim!: productive.cases intro: productive.intros(1))
...
lemma lfinite_lshift[simp]:
  \<open>lfinite (xs @@- lxs) \<longleftrightarrow> lfinite lxs\<close>
  by (induction xs) simp_all

lemma productive_coinduct_prepend_cong1:
  assumes H1: "X lxs" 
    and H2:  "(\<And>x1.
    X x1 \<Longrightarrow>
    ...
    subgoal for ys xs
      ...
      subgoal for a list
        ...
        subgoal for t' d'
          ...
          subgoal
            apply (simp add: productive_prepend_cong1_prepend_1)
            apply (metis UnCI drop0 insert_iff mem_Collect_eq nth_Cons_0 vimage_def zero_less_Suc)
            done
          subgoal
            apply (simp add: productive_prepend_cong1_prepend_1)
            apply (metis UnCI drop0 insert_iff mem_Collect_eq nth_Cons_0 vimage_def zero_less_Suc)
...
lemma productivity_good_example: "productive good_example"
  using productive.simps by fastforce
...
next
  case (Suc n)
  then show ?case
    by (metis ldrop_eSuc_ltl ldrop_enat ltl_ldropn ltl_simps(1) ltl_simps(2) neq_LNil_conv productive_drop_head)
qed

lemma productive_ldrop':
  \<open>productive lxs \<Longrightarrow> lxs' = ldrop n lxs \<Longrightarrow> productive lxs'\<close>
  by (cases n) (simp_all add: productive_ldrop productive.intros(1))

lemma productive_LCons_Data_obtain_Watermark:
  assumes \<open>productive (LCons (Data t d) lxs)\<close> \<open>\<not> lfinite (LCons (Data t d) lxs)\<close>
  obtains wm where \<open>wm \<in> Watermark -` lset lxs\<close> \<open>wm \<ge> t\<close>
  using productive.cases assms by force

lemma productive_lnth_Data_obtain_Watermark:
  assumes \<open>productive lxs\<close> \<open>\<not> lfinite lxs\<close> \<open>lnth lxs n = Data t d\<close>
  obtains m wm where \<open>m > n\<close> \<open>wm \<ge> t\<close> \<open>lnth lxs m = Watermark wm\<close>
proof atomize_elim
  obtain lxs' where 1: \<open>ldrop (enat n) lxs = LCons (Data t d) lxs'\<close>
    using assms(2,3) enat_ord_code(4) ldrop_enat ldropn_Suc_conv_ldropn not_lfinite_llength by metis
  have \<open>\<not> lfinite (ldrop (enat n) lxs)\<close>
    using assms(2) by simp
  moreover have \<open>productive (ldrop (enat n) lxs)\<close>
    using assms(1) productive_ldrop by blast
  ultimately obtain wm where \<open>wm \<in> Watermark -` lset lxs'\<close> and 2: \<open>wm \<ge> t\<close>
    using productive_LCons_Data_obtain_Watermark 1 by metis
  then obtain m where \<open>lnth lxs' m = Watermark wm\<close>
    by (meson in_lset_conv_lnth vimage_eq)
  thus \<open>\<exists>m wm. n < m \<and> t \<le> wm \<and> lnth lxs m = Watermark wm\<close>
    by (metis (no_types, lifting) 1 2 assms(2) enat_ord_code(4) ldrop_enat ldropn_Suc_conv_ldropn lessI
        llength_eq_infty_conv_lfinite llist.inject lnth_ldropn trans_less_add2)
qed
...
lemma strict_monotone_productive_lfinite_lfilter_eq_t:
  ...
  subgoal
    ...
    subgoal for d n
      apply (erule productive_lnth_Data_obtain_Watermark[where t=t and d=d and n= n])
      apply assumption+
      subgoal for m wm
        apply (simp add: not_lfinite_llength)
  ...
  subgoal
    ...
    apply (rule ldropn_lfinite_lfilter[where n="0"])
    apply (simp add: not_lfinite_llength)
    apply simp
...
definition "ts' lxs t = time ` set (list_of (lfilter (\<lambda> x . case x of Data t' d \<Rightarrow> t' \<le> t | Watermark wm \<Rightarrow> False) lxs))"
...
lemma ts_shift_map_Data[simp]:
  ...
  by (auto simp add: ts_def split_beta split: prod.splits)
...
lemma img_time_Lambda_Data:
  "finite A \<Longrightarrow> time ` (\<lambda>x. Data x (f x)) ` A = A"
...
lemma productive_Data_in_ge_Watermark_in:
  ...
  by (metis enat_ord_code(4) in_lset_conv_lnth llength_eq_infty_conv_lfinite productive_lnth_Data_obtain_Watermark)
...
lemma monotone_llist_of:
  "(\<forall> x \<in> set xs. is_Data x \<and> (\<forall> wm \<in> WM. \<not> time x \<le> wm)) \<Longrightarrow>
