...
(* Missing one-step preservation facts needed by the loop_updates induction.
   They are stated separately so we can choose an order for proving them before
   attempting the main loop invariant. *)
