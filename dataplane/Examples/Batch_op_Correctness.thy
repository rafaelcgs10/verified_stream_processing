...
definition "produ_supported su os c = (\<forall> nid p t m. (p, t, m) \<in> set (produ (os nid)) \<longrightarrow> (\<exists> t' l s. zcount (c_pts c l) t' > 0 \<and> t = t' + s \<and> s \<in>\<^sub>A graph.path_weight su l (Loc nid (Src p))))"
definition "consu_supported su os c = (\<forall> nid p t m. (p, t, m) \<in> set (consu (os nid)) \<longrightarrow> (\<exists> t' l s. zcount (c_pts c l) t' > 0 \<and> t = t' + s \<and> s \<in>\<^sub>A graph.path_weight su l (Loc nid (Src p))))"

definition "dataplane_tracker_inv os cbufs sg = 
   (\<exists> c c' cgs chns caps.
     c = pt_tr sg \<and>
     cgs = extract_prog Enum.enum (edges sg) os \<and>
     ...
     changes_above_impl_inv (summ sg) (change_multiplicities (summ sg) (extract_progress  nid (edges sg) (snd (obtain_progress (os nid)))) c)
     (extract_progress nid' (edges sg) (snd (obtain_progress (os nid'))))) \<and>
     consu_supported (summ sg) os c)"

lemma zmset_map_filter_Trg_extract_prog:
  "zmset (map snd (filter (\<lambda>(l', t, d). Loc nid (Trg p) = l') (extract_prog Enum.enum (edges sg) os))) = 
...
find_consts "_ set \<Rightarrow> _ set" name: antich

definition "graph_summar_edges_inv su ed os = (
  ...
  (\<forall> nid p l s. l \<noteq> (Loc nid (Trg p)) \<longrightarrow> (s \<in>\<^sub>A graph.path_weight su (Loc nid (Trg p)) l \<longleftrightarrow> (\<exists> t' p'' s'. s = t' -+- s' \<and> t' \<in> set (summar (os nid) p p'') \<and> s' \<in>\<^sub>A graph.path_weight su (Loc nid (Src p'')) l))) \<and>
  (\<forall> nid p l s. l \<noteq> (Loc nid (Src p)) \<longrightarrow> (s \<in>\<^sub>A graph.path_weight su (Loc nid (Src p)) l \<longleftrightarrow> (\<exists> nid' p'. (nid', p') \<in> set (ed (nid, p)) \<and> s \<in>\<^sub>A graph.path_weight su (Loc nid (Trg p')) l))) \<and>
  (\<forall> nid p p'. distinct (summar (os nid) p p')) \<and>
  (\<forall> nid p p'. \<forall> t \<in> set (summar (os nid) p p'). \<not> (\<exists> t' \<in> set (summar (os nid) p p'). t' < t))
  )"
...
lemma sorried1:
  "frontier_less_equal (frontier (\<Sum>loc'\<in>insert x A. f loc' (frontier (g loc')))) t' \<Longrightarrow>
   g x = g' x + {#t#}\<^sub>z \<Longrightarrow>
   A = C \<union> B \<Longrightarrow>
   (\<forall> loc'\<in> C. f loc' (frontier (g loc')) = f loc' (frontier (g' loc'))) \<Longrightarrow>
   frontier_less_equal (frontier (\<Sum>loc'\<in>insert x A. f loc' (frontier (g' loc')))) t'"
  oops
...
lemma
  "dataplane_tracker_inv os cbufs sg \<Longrightarrow>
   ...
   graph_summar_edges_inv (summ sg) (edges sg) os \<Longrightarrow>
  ...
  subgoal for c c' cgs chns caps
    ...
    subgoal premises prems
      ...
      subgoal
        apply (subgoal_tac
            "zmset (map snd (filter (\<lambda>(l', t, d). Loc nid (Trg p) = l') (extract_prog Enum.enum (edges sg) (os(nid := consumes (os nid) p t d))))) =
   zmset (map snd (filter (\<lambda>(l', t, d). Loc nid (Trg p) = l') (extract_prog Enum.enum (edges sg) os))) - {#t#}\<^sub>z")
        subgoal
          by auto
        subgoal premises
          ...
          subgoal for X
            apply (cases X)
              ...
              subgoal for  p' m
                ...
                apply (subst (asm) comm_monoid_add_class.sum.subset_diff[where B="{Loc nid (Trg p)} \<union> (\<lambda> p'. Loc nid (Src p')) ` {p'. summar (os nid) p p' \<noteq> []}"])
                  apply simp_all
                 apply force
                ...
                apply (elim disjE)
                 defer
                subgoal
                  apply (clarsimp simp add: split_beta zmset_map_snd_concat_sum  comp_def filter_concat c_pts_change_multiplicities extract_prog_def)
                  apply (subst Propagate.dataflow_topology.implied_frontier_alt_def)
                  using prems(2) apply assumption
                  apply (rule frontier_less_equal_subset_sumI[of _ _ "{Loc nid (Trg p)} \<union> (\<lambda> p'. Loc nid (Src p')) ` {p'. summar (os nid) p p' \<noteq> []}"])
                     apply simp_all
                   ...
                   apply force
                  apply (clarsimp simp add: comm_monoid_add_class.sum.distrib split_beta zmset_map_snd_concat_sum  comp_def filter_concat c_pts_change_multiplicities extract_prog_def)
                  apply (subst (asm) frontier_less_equal_iff2)
                  apply clarsimp
                  subgoal for ft
                    apply (cases "\<exists> s p'' t''. t'' \<in> set (summar (os nid) p p'') \<and> s \<in>\<^sub>A graph.path_weight (summ sg) (Loc nid (Src p'')) (Loc nid' (Trg p')) \<and> ft = t -+- t'' -+- s")
                    subgoal
                      apply clarsimp
                      subgoal for s p'' t''
                        apply (subst (asm) comm_monoid_add_class.sum.insert_remove)
                         apply simp_all
                        apply (drule in_frontier_sum)
                        subgoal
                        ...
                        subgoal
                          ...
                          subgoal
                            ...
                            subgoal
                              apply (rule disjI2)
                              apply (rule frontier_less_equal_sumI[of _ _ "Loc nid (Src p'')"])
                                 apply simp_all
                              subgoal
                                apply clarsimp
                                apply (metis (no_types) AP_simp after_summary_def dataflow_topology_from_tree.after_summary_zmset_of_nonneg)
                                done
                               apply fastforce
                              apply (rule frontier_less_equal_sumI[of _ _ s])
                                 apply blast
                                apply clarsimp
                              using member_antichain.rep_eq apply blast
                              apply (subst frontier_less_equal_iff2)
                              ...
                              subgoal
                                apply (rule in_frontier_sumI2)
                                subgoal 
                                  apply (clarsimp simp only: to_zmset_map in_frontier_iff)
                                  apply (intro conjI)
                                  subgoal
                                    apply (subgoal_tac "\<not> (\<exists>t'\<in>set (summar (os nid) p p''). t' < t'')")
                                    subgoal premises prems3
                                      using prems3(4, 10) apply -
                                      apply (auto simp flip: to_zmset_map simp add: zcount_to_zmset)
                                      done
                                    subgoal premises prems3
                                      using prems(3) prems3(4) 
                                      unfolding graph_summar_edges_inv_def
                                      by auto
                                    done
                                  subgoal
                                    by (auto simp flip: to_zmset_map simp add: zcount_to_zmset)
                                  done
                                subgoal
                                  apply safe
                                  subgoal premises prems2 for tt
                                    using prems2(3,4,7,9,11-) apply -
                                    apply (drule spec[of _ "tt -+- s"])
                                    apply (drule mp)
                                    subgoal
                                      apply (simp add: zcount_sum)
                                      apply (rule Timely_Infrastructure.dataflow_topology_from_tree.sum_pos[of _ _  "Loc nid (Src p'')"])
                                         apply simp_all
                                      subgoal
                                      by (meson nonneg_zcount_image_zmset sum_nonneg zcount_zmset_of_nonneg)
                                    subgoal
                                      by (force simp add: image_iff)
                                    subgoal
                                      apply (rule Timely_Infrastructure.dataflow_topology_from_tree.sum_pos[of _ _ s])
                                         apply simp_all
                                      using prems2(5) member_antichain.rep_eq apply blast
                                      using prems2(10) apply -
                                      apply (rule pos_zcount_image_zmset)
                                       apply simp_all
                                      apply (subgoal_tac "\<forall> t. zcount (c_pts (pt_tr sg) (Loc nid (Src p''))) t \<ge> 0")
                                      subgoal
                                        apply (auto simp flip: member_antichain.rep_eq simp add: in_frontier_iff)                                        
                                        subgoal for ft'
                                          using prems2(7)
                                        ...
                                        subgoal
                                          using add_strict_increasing2 by blast

                                      find_theorems  "_ \<in>\<^sub>A frontier _ \<longleftrightarrow> _"
                            ...
                            subgoal
                              sorry
                            subgoal
                              by (meson to_zmset_nenneg)
                              


                      find_theorems "_ \<in>\<^sub>A (frontier (_ + _))"
               ...
               apply (rule frontier_less_equal_sumI[of _ _ s])
                       apply force 
                      apply simp
                    using set_antichain2 apply blast

                find_theorems frontier sum frontier_less_equal


              apply (clarsimp simp add: c_pts_change_multiplicities split_beta)
              ...
              subgoal for l s t''
                ...
                subgoal
                  ...
                  subgoal premises prems2
                    ...
                    apply (simp add: extract_progress_def comm_monoid_add_class.sum.distrib obtain_progress_def)
                    apply (subst (asm) filter_False)



                find_theorems "sum _ _ = 0" 
...
lemma
  "frontier_less_equal (ifrontier su (-+-) (change_multiplicities su xs c) l) t \<Longrightarrow>
   dataflow_topology su (-+-) \<Longrightarrow>
   changes_above_impl_inv su (change_multiplicities su xs c) ys \<Longrightarrow>
   frontier_less_equal (ifrontier su (-+-) (change_multiplicities su (xs @ ys) c) l) t"
  ...
  unfolding change_multiplicities_append_comp
  apply simp
  unfolding changes_above_impl_inv_def
          ...
          find_theorems ifrontier change_multiplicities


end
          apply (subgoal_tac "changes_above_impl_inv (summ sg) (pt_tr sg) (extract_prog enum_class.enum (edges sg) (os(nid := consumes (os nid) p t d)))")
          subgoal
            apply (subst (asm) changes_above_impl_inv_def)
            apply (drule bspec[of _ _ "(Loc nid (Trg p), t, -1)"])
            subgoal premises
              unfolding extract_prog_def extract_progress_def obtain_progress_def consumes_def
              by (auto simp add: split_beta enum_class.enum_UNIV)
            apply simp
            apply (drule frontier_less_equal_ifrontierE)
            subgoal
              using prems(2) by assumption
            apply clarsimp
            subgoal for l s t'
              apply (simp add: c_pts_change_multiplicities)
              apply (subst (asm) (2) frontier_less_equal_iff2)
              ...
              subgoal for t''
                ...
                subgoal
                  ...
                  subgoal
                    apply hypsubst_thin
                    apply (drule Graph.graph.path_weight_conv_path[OF dataflow_topology.axioms(1)[OF prems(2)]])
                    apply clarsimp
                    subgoal for xs
                      apply (cases xs)
                      subgoal
                        apply simp
                        apply hypsubst_thin
                        apply (drule empty_path_inversion)
                        using dataflow_topology.axioms(1)[OF prems(2)] apply assumption
                        apply hypsubst_thin
                        apply (drule frontier_less_equal_ifrontierE)
                        subgoal
                          using prems(2) by assumption
                                ...
                                subgoal
                                  ...
                                  subgoal for p'
                                    ...
                                    subgoal
                                      apply clarsimp
                                      subgoal for t''
                                        apply (rule frontier_less_equal_ifrontierI[OF prems(2), of 0, where l="Loc nid' (Src p')", simplified])
                                        subgoal
                                          sorry
                                        subgoal
                                          apply (simp add:  c_pts_change_multiplicities)
                                          ...
                                          subgoal
                                            apply (rule disjI1)
                                            unfolding frontier_less_equal_iff2
                                            apply (meson frontier_less_equal_iff2 frontier_less_equal_trans frontier_less_equal_zcount_pos)
                                            done
                                          subgoal
                                            sorry
                                          subgoal premises prems2
                                    ...
                                    subgoal
                                      ...
                                      subgoal for m'
                                        ...
                                        subgoal premises prems2
                                          ...
                                          apply (force simp add: image_iff split_beta enum_class.enum_UNIV)
                                          done
                                        apply (drule frontier_less_equal_ifrontierE[where t=t'''])
                                        subgoal
                                          using prems(2) by assumption
                                        apply clarsimp
                                        subgoal for l2 s2 t4
                                          apply (cases "l2 = Loc nid (Trg p)")
                                          subgoal
                                            apply hypsubst_thin
                                            apply (drule Graph.graph.path_weight_conv_path[OF dataflow_topology.axioms(1)[OF prems(2)]])
                                            apply clarsimp
                                            subgoal for xs
                                              ...
                                              subgoal
                                                (*
  make frontier_less_equal_add_cases_stronger even stronger to use t4

*)
                                                apply (clarsimp simp add: split_beta)
                                                apply hypsubst_thin
                                                apply (rule frontier_less_equal_ifrontierI[OF prems(2), of 0, where l="Loc nid (Trg p)", simplified])
                                                subgoal sorry
                                                apply (simp add:  c_pts_change_multiplicities)
                                                apply (rule frontier_less_equal_addI1)
                                                 apply assumption
                                                subgoal
                                                  unfolding extract_progress_def obtain_progress_def extract_prog_def
                                                  apply (auto 0 0 simp add: image_iff split_beta enum_class.enum_UNIV)
                                                  subgoal for tc
  ...
  subgoal for x xs
    apply hypsubst_thin
    apply (rule frontier_less_equal_ifrontierI[OF prems(2),  where l=l2])
    subgoal
      sorry
    apply (simp add:  c_pts_change_multiplicities)
    apply (rule frontier_less_equal_addI1)
  ...
  apply (drule Propagate.dataflow_topology.no_zero_cycle[OF prems(2), where loc="Loc nid (Trg p)" and xs=xs])
  apply simp_all

find_theorems dataflow_topology  Graph.graph

find_theorems graph.path_weight graph.path name: path_weight_conv_pat

thm Propagate.dataflow_topology.no_zero_cycle[OF prems(2), where loc=l]


find_theorems  0 name: Propagate name: zero

  oops


lemma
  "changes_above_impl_inv su c (A @ xs @ ys) \<Longrightarrow>
   changes_above_impl_inv su (change_multiplicities su xs c) A \<Longrightarrow>
   changes_above_impl_inv su (change_multiplicities su (xs @ ys) c) A"
  unfolding change_multiplicities_append_comp
  apply simp

  find_theorems changes_above_impl_inv
  ...
  apply (auto simp add: distinct_map enum_class.enum_distinct enum_class.enum_UNIV)
...
lemma frontier_plus_cong:
  "frontier B = frontier C \<Longrightarrow>
   (\<forall> t. zcount A t \<ge> 0) \<Longrightarrow>
   (\<forall> t. zcount B t \<ge> 0) \<Longrightarrow>
   (\<forall> t. zcount C t \<ge> 0) \<Longrightarrow>
   frontier (A + B) = frontier (A + C)"
  apply (auto simp add: frontier_def ac_eq_iff)
  subgoal for t
    apply (subst in_antichain_minimal_antichain)
    apply (metis (mono_tags, lifting) finite_zcount_pos mem_Collect_eq rev_finite_subset subsetI zcount_union)
    apply (subst (asm) in_antichain_minimal_antichain)
    apply (metis (mono_tags, lifting) finite_zcount_pos mem_Collect_eq rev_finite_subset subsetI zcount_union)
    unfolding in_minimal_antichain
    apply auto
    subgoal
      by (metis int_sum_disj)
    subgoal
      by (smt (verit, ccfv_threshold) order_le_less_trans order_zmset_exists_foundation')
    done
  subgoal for t
    apply (subst in_antichain_minimal_antichain)
    apply (metis (mono_tags, lifting) finite_zcount_pos mem_Collect_eq rev_finite_subset subsetI zcount_union)
...
lemma
  ...
  apply simp_all
  apply (rule frontier_plus_cong)
  apply simp_all
  defer
  apply (meson dataflow_topology.mset_neg_after_summary mset_neg_empty_iff mset_neg_sum_set neg_zmset_pos)
  apply (simp_all add: dataflow_topology.mset_neg_after_summary mset_neg_zcount_nonneg)
  ...
  apply assumption+
  apply simp 
  apply (drule spec[of _ "(zmset_of (mset_pos ((c_pts c la) - A)))"])
  apply (simp add: )
...
lemma
  ...
  using data_in_channel_justifies_c_pts[OF prems(4) prems(5), where t=t and nid=nid and p=p] apply -
  apply (drule meta_mp)
  subgoal premises
    using prems(1)
  ...
  subgoal
    (*
  nid' can only cause changes in nid with produ
*)
    sorry
  subgoal
    apply clarsimp
...
*)
    ...
    subgoal for nid'' p''
      ...
      subgoal
        ...
        find_theorems extract_progress set
        oops
