...
definition "produ_supported os c = (\<forall> nid p t m. (p, t, m) \<in> set (produ (os nid)) \<longrightarrow> (\<exists> t'. zcount (c_pts c (Loc nid (Src p))) t' > 0 \<and> t' \<le> t) \<or> (\<exists> m. (p, t, m) \<in> set (inter (os nid)) \<and> m > 0))"
