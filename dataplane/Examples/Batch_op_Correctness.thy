...
"init_operator_state_ty2 su \<equiv> \<lparr> 
   ...
   de2 = projr
   \<rparr>"
abbreviation "l2 f \<equiv> Logic (batch_op (init_operator_state_ty2 default_internal_summary) f) default_internal_summary"

abbreviation "test_dt2 \<equiv> Comp [(0, 1) \<mapsto> (0, 1)] (l1 (\<lambda> _. inps2)) (l2 (\<lambda> b. if b = [] then trace (STR ''Empty batch! ! !'') [] else [Max (set b)]))"

abbreviation "test_op \<equiv> compile_dataflow test_dt2 :: (2 \<times> 1, 2 \<times> 1, _) op"
...
lemma correctness_gen:
  ...
  shows 
    \<open>set_op S D
    (dataflow_op sg (G_op ip_state (\<lambda> p. case p of Inl x \<Rightarrow> [] | Inr x \<Rightarrow> map (\<lambda> (d, t). Inr (Inl d, t)) cbuf) bt_state f)) \<approx>
    set_spec_op (cUn S S') D\<close>
  oops
