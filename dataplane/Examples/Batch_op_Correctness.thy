...
record ('nid, 'p, 't, 'd) timely_state =
  ...
  opinp :: "'nid \<Rightarrow> 'p \<Rightarrow> ('d \<times> 't) buf"
  opfun :: "'nid \<Rightarrow> ('p \<Rightarrow> 't antichain) \<Rightarrow> ('p \<Rightarrow> ('d \<times> 't) buf) \<Rightarrow> ('p \<Rightarrow> ('d \<times> 't) buf)"
...
find_consts " _ zmultiset" "_ antichain"

term "(-++-)"

find_consts name: result

definition consumes_action where
  "consumes_action summary ts ts' = (\<exists> nid pfx sfx icaps' icaps''.
   (\<forall> p. pfx p @ sfx p = cbufs ts nid p) \<and>
   ...
   icaps'' = (\<lambda> l l'. case (l, l) of (Loc nid' (Trg p'), Loc nid'' (Src p'')) \<Rightarrow> if nid = nid' then to_zmset (map snd (pfx p')) else {#}\<^sub>z | _ \<Rightarrow> {#}\<^sub>z) \<and>
   ts' = ts\<lparr> 
     cbufs := (cbufs ts)(nid := \<lambda> p. sfx p),
     opinp := (opinp ts)(nid := \<lambda> p. opinp ts nid p @ sfx p),
     icaps := (\<lambda> l. icaps ts l - icaps' l)
    \<rparr>)"
