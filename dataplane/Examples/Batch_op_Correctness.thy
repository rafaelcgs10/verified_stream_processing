...
definition "produ_supported su os c = (\<forall> nid p t m. (p, t, m) \<in> set (produ (os nid)) \<longrightarrow> (\<exists> t' l s. zcount (c_pts c l) t' > 0 \<and> t \<ge> t' + s \<and> s \<in>\<^sub>A graph.path_weight su l (Loc nid (Src p))))"
definition "consu_supported su os c = (\<forall> nid p t m. (p, t, m) \<in> set (consu (os nid)) \<longrightarrow> (\<exists> t' l s. zcount (c_pts c l) t' > 0 \<and> t = t' + s \<and> s \<in>\<^sub>A graph.path_weight su l (Loc nid (Trg p))))"
...
definition "dataplane_tracker_inv os cbufs sg = 
   (\<exists> c c' cgs chns caps.
     ...
     (\<forall> nid. produ_supported (summ sg) os (change_multiplicities (summ sg) (extract_progress nid (edges sg) (snd (obtain_progress (os nid)))) c)))"
...
lemma zmset_filter_extract_progress_Src_consumes_diff:
  "nid' \<noteq> nid \<Longrightarrow>
   ...
   {#}\<^sub>z"
  by (clarsimp simp add: split_beta extract_progress_def obtain_progress_def filter_concat filter_map map_concat comp_def zmset_concat)
   ...
   zmset (map snd (filter (\<lambda>(l', t, d). Loc nid (Trg p) = l') (extract_prog Enum.enum (edges sg) os))) - {#t#}\<^sub>z")
    ...
    subgoal premises prems
      ...
      subgoal  for nid'
                      ...
                      subgoal
                        ...
                        subgoal premises prems2
                          ...
                          subgoal
                            ...
                            apply (auto 0 0 simp add: filter_concat comp_def map_concat zmset_concat split_beta)
                            done
                          apply simp
        ...
        (mset_set
          (set_antichain
            (frontier (c_pts (change_multiplicities (summ sg) (extract_progress nid (edges sg) (snd (obtain_progress (consumes (os nid) p t d)))) (pt_tr sg)) _))))"
                            "graph.path_weight (summ sg) _ (Loc nid' (Trg p'))"]
                          after_summary_def
                          dataflow_topology_from_tree.after_summary_zmset_of_nonneg[of
                            "mset_set
        (set_antichain (frontier (c_pts (change_multiplicities (summ sg) (extract_progress nid (edges sg) (snd (obtain_progress (consumes (os nid) p t d)))) (pt_tr sg)) _)))"
                            "graph.path_weight (summ sg) _ (Loc nid' (Trg p'))"])
                    subgoal
                      apply clarsimp
                      apply (elim disjE conjE exE)
                                    ...
                                    subgoal
                                      ...
                                      subgoal
                                        ...
                                        subgoal for s'' ft3
                                          ...
                                          apply (drule in_frontier_minusI)
                                          apply assumption
                                            apply (cases "ft2 = ft3")
                                            subgoal
                                              apply hypsubst_thin
                                              apply simp
                                            apply (subgoal_tac "\<not> s'' < t'' -+- s'")
                                            subgoal
                                              by (meson add_le_cancel_right add_less_imp_less_left basic_trans_rules(21))
                                            subgoal
                                              apply (drule path_weight_incomparable[rotated])
                                              apply assumption
                                              subgoal 
                                                apply (rule dataflow_topology.axioms(1))
                                                apply (rule prems(2))
                                          ...
                                          subgoal
                                            apply (subgoal_tac "\<not> s'' < t'' -+- s' \<and> \<not> t'' -+- s' < s'' \<and> \<not> ft2 < ft3 \<and> \<not> ft3 < ft2")
                                            subgoal premises prems2
                                              using prems2(9,10,12,13,16,17) apply -
                                              apply clarsimp

                                              find_theorems name: resul name: mono
    ...
    find_theorems "_ + (_ - _) = _"
                                    ...
                                    find_theorems zcount image_zmset

end
                                  subgoal
                                    by (metis group_cancel.add1)

                                find_theorems "_ \<in>\<^sub>A frontier {#?f t . t \<in>#\<^sub>z _#}"
    ...
    find_theorems frontier sum name: D
...
lemma
  ...
  find_theorems "0 < zcount (image_zmset _ _) _"


end
  apply (subgoal_tac "\<not> t'' < t2' \<and> \<not> t2' < t'' \<and> \<not> s' < s \<and> \<not> s < s'")
  subgoal
    apply (subst (asm) (3 4) add.assoc)
    apply auto   
      (* ? ? ? *)
    sorry
  subgoal
  ...
  apply simp_all
  using set_antichain2 apply auto[1]
  subgoal for i
    apply (subgoal_tac "i = s")
    defer
    subgoal
      apply (simp flip: member_antichain.rep_eq)
  ...
  find_theorems "_ \<in>\<^sub>A frontier (sum _ _)"
