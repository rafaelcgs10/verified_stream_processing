...
(* Interpreting the timestamps as deltas:
...
corec events_to_pairs where
  ...
  | LCons (Data t d) lxs' \<Rightarrow>
    ...
    in LCons (d, n' + t) (events_to_pairs n' lxs'))\<close>
...
proof
  ...
  | LCons (Data t d) lxs' \<Rightarrow>
    let n' = foldl (+) n (list_of (lmap event.time (ltakeWhile (Not \<circ> is_Data) lxs)))
    in LCons (d, n' + t) (events_to_pairs n' lxs'))
  = LNil\<close>
...
lemma events_to_pairs_LCons_Data:
  \<open>events_to_pairs n (LCons (Data t d) lxs) = LCons (d, n + t) (events_to_pairs n lxs)\<close>
  by (subst events_to_pairs.code) simp

lemma events_to_pairs_LCons_Watermark:
  \<open>events_to_pairs n (LCons (Watermark wm) lxs) = events_to_pairs (n + wm) lxs\<close>
  by (subst (1 2) events_to_pairs.code)
    (simp add: lfinite_ltakeWhile list_of_LCons_conv split: llist.splits event.splits)

lemma event_to_Data:
  assumes \<open>events_to_pairs n lxs = LCons x lxs'\<close>
  obtains t d lxs'' n' where \<open>ldropWhile (Not \<circ> is_Data) lxs = LCons (Data t d) lxs''\<close>
    \<open>n' = foldl (+) n (list_of (lmap event.time (ltakeWhile (Not \<circ> is_Data) lxs)))\<close>
    \<open>x = (d, n' + t)\<close> \<open>lxs' = events_to_pairs n' lxs''\<close>
proof atomize_elim
  ...
  thus \<open>\<exists>t d lxs'' n'.
       ldropWhile (Not \<circ> is_Data) lxs = LCons (Data t d) lxs'' \<and>
       n' = foldl (+) n (list_of (lmap event.time (ltakeWhile (Not \<circ> is_Data) lxs))) \<and>
       x = (d, n' + t) \<and> lxs' = events_to_pairs n' lxs''\<close>
    by (auto split: llist.splits event.splits dest: ldropWhile_LConsD)
qed
