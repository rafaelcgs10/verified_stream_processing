...
proof (induction \<open>list_of (ltakeWhile (Not \<circ> is_Data) (es os p))\<close> arbitrary: os)
  ...
  ultimately show ?case
    using Nil(4) timely_monotone.cases by force
next
  case (Cons x xs)
  show ?case
  proof (cases x)
  ...
  next
    ...
    moreover have \<open>timely_monotone (es ?os' p) (mset (ocaps ?os' p))\<close>
      by simp (smt (verit) Cons(5) head_es mset_remove_last timely_monotone.cases event.distinct(2,5) event.simps(2) lhd_LCons_ltl_es llist.simps(1,2))
  ...
  next
    ...
    moreover have \<open>timely_monotone (es ?os' p) (mset (ocaps ?os' p))\<close>
      apply simp
      using Cons(5) head_es timely_monotone.cases lhd_LCons_ltl_es by force
    moreover have \<open>os' = foldl (ooo_input_os_Drop_Mint p) (?os'\<lparr>es := (es ?os')(p := lxs)\<rparr>) (list_of (ltakeWhile (Not \<circ> is_Data) (es ?os' p)))\<close>
    proof -
...
proof (induction \<open>ltakeWhile (Not \<circ> is_Data) (es os p)\<close> arbitrary: os op rule: lfinite_induct)
  ...
  moreover from this have \<open>ocaps os p \<noteq> []\<close>
    using LNil(2,6) timely_monotone.cases by force
...
next
  ...
  have ocaps_not_empty: \<open>ocaps os p \<noteq> []\<close>
    using LCons(8) head_es(3) timely_monotone.cases by force
  ...
  proof -
    ...
    next
      ...
      hence \<open>ocaps (ooo_input_os_Drop_Mint p (os\<lparr>es := map_entry p ltl (es os)\<rparr>) e) p = remove_last t' (ocaps os p)\<close>
        by simp
      thus ?thesis
       using Drop LCons(8) ocaps_not_empty head_es(3) es_os1 timely_monotone.cases
       mset_remove_last event.distinct(2,5) event.inject(2) llist.simps(1) mset_zero_iff
       by (smt (verit, ccfv_threshold))
    next
      case Mint
      thus ?thesis
        using LCons(8) head_es(3) timely_monotone.cases by fastforce
    qed
...
lemma ooo_input_op_source_op:
  ...
  timely_monotone (es os 0) (mset (ocaps os 0)) \<Longrightarrow>
...
proof (coinduction arbitrary: sg os rule: wbisim_coinduct_upto'')
  ...
  then show ?case
    ...
    subgoal
      ...
      subgoal
        using timely_monotone.cases by fastforce
      subgoal
        using timely_monotone.cases by blast
      subgoal
        using timely_monotone.cases mset_remove_last event.simps(2,5,9) llist.simps(1) mset_zero_iff
        by (smt (verit))
      subgoal
        using timely_monotone.cases by fastforce
      done
