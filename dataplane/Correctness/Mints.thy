...
lemma
  ...
  subgoal premises prems
   ...
   apply (subgoal_tac "zcount (zmset (map snd (filter ((=) p \<circ> fst) (operator_state.inter (os nid))))) t'' > 0 \<or> zcount (c_pts (pt_tr sg) (Loc nid (Src p))) t'' > 0")
      ...
      subgoal premises temp2
        using temp2(3,5-) apply -
          apply (induct "length (filter (\<lambda> (p', t', m). p' = p \<and> t' \<le> t) (operator_state.inter (os nid)))" arbitrary: t os rule: less_induct)
        subgoal for t os
        apply (cases "t'' = t")
        subgoal
          apply simp
      ...
      subgoal
        ...
        apply (drule meta_spec[of _ "os( nid := (os nid)\<lparr> inter := filter (\<lambda> (p', t', m). p' = p \<and> t' < t'') (operator_state.inter (os nid)) \<rparr> )"])
        apply (drule meta_mp)
          subgoal
            apply (drule gt_0_zcount_msetD)
            apply clarsimp
          ...
          subgoal
            ...
            using dual_order.trans temp(3) apply blast
            subgoal for t2 t3 p' s m''
              using temp(3) 
