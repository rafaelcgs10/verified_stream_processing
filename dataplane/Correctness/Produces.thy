...
definition zero_predecessors where
  "zero_predecessors t loc  = {loc'. (\<exists> s . s \<in>\<^sub>A graph.path_weight summary loc' loc \<and> loc \<noteq> loc' \<and> results_in t s = t)}"
...
function weight' where
  ...
  by auto
termination
  apply(relation "{(loc', loc) . \<exists> s . s \<in>\<^sub>A graph.path_weight summary loc' loc \<and> loc \<noteq> loc' \<and> results_in t s = t}")
  subgoal
    ...
    proof -
      have "(loc', loc) \<in> {(loc', loc). \<exists>s. s \<in>\<^sub>A graph.path_weight summary loc' loc \<and> loc \<noteq> loc' \<and> results_in t s = t}\<^sup>+ \<Longrightarrow>
...
lemma sum_weight'_lt:
  assumes D: "dataflow_topology su (-+-)"
  shows "finite B \<Longrightarrow>
   A \<subset> B \<Longrightarrow>
   sum (dataflow_topology.weight' su (-+-) t) A < sum (dataflow_topology.weight' su (-+-) t) B"
   apply (subst (2) Groups_Big.comm_monoid_add_class.sum.subset_diff[of "B - A"])
      apply blast
     apply simp_all
  ...
  apply (rule Groups_Big.ordered_comm_monoid_add_class.sum_pos)
    apply simp_all
  apply (subst dataflow_topology.weight'.simps[OF D])
  apply simp
  done
 ...
 (\<Sum>x\<in>UNIV. zmset (map snd (filter (\<lambda>(p''a, ab). graph_to_nxt (summ sg) (fst x, p''a) = Some (nid'', p'') \<and> snd x = p''a) (produ (os (fst x)))))) = 
                              ...
                              subgoal
                                ...
                                subgoal
                                  ...
                                  subgoal for m''
                                    ...
                                    apply (elim disjE)
                                    subgoal
                                      apply (drule conjunct1[OF conjunct2[OF temp(5)[unfolded graph_summar_nt_def]], rule_format])
                                      apply (drule path_weight_direct_0path[OF Propagate.dataflow_topology.axioms(1)[OF D]])
                                      apply (drule Graph.graph.path_weight_elem_trans[OF Propagate.dataflow_topology.axioms(1)[OF D], of 0])
                                       apply assumption
                                      apply clarsimp
                                      subgoal for u
                                      apply(rule frontier_less_equal_trans[of _ "t' -+- u"]; simp?)
                                      apply (rule frontier_less_equal_ifrontierI[of _ u "Loc nid''' (Src p''')", simplified, OF D])
                                        apply assumption
                                      ...
                                      subgoal for m''
                                        ...
                                        subgoal for t'
                                          ...
                                          subgoal
                                           ...
                                           apply simp
                                          using frontier_less_equal_trans frontier_less_equal_zcount_pos apply blast
                                          apply (meson add_mono order.trans)
                                          done
                                        done
                                        subgoal for t1
                                          apply clarsimp
  ...
  < (card {t. t \<le> t' \<and> (\<exists>p m nid. (p, t, m) \<in> set (consu (os nid)))}, dataflow_topology.weight' (summ sg) (-+-) t' (Loc nid'' (Trg p'')))")
                                            ...
                                            subgoal
                                              ...
                                              subgoal
                                                ...
                                                subgoal premises aux2
                                                  apply (subst dataflow_topology.weight'.simps[OF D])
                                                  apply simp
                                                  apply (subst dataflow_topology.zero_predecessors_def[OF D])
                                                  apply simp
                                                  apply (subgoal_tac "{loc'. 0 \<in>\<^sub>A graph.path_weight (summ sg) loc' (Loc nid''' (Trg p1)) \<and> Loc nid''' (Trg p1) \<noteq> loc'} \<subset> {loc'. 0 \<in>\<^sub>A graph.path_weight (summ sg) loc' (Loc nid'' (Trg p'')) \<and> Loc nid'' (Trg p'') \<noteq> loc'}")
                                                  subgoal
                                                    apply (rule sum_weight'_lt[OF D])
                                                     apply simp
                                                    apply auto
                                                    done
                                                  subgoal
                                                    apply (subgoal_tac "0 \<in>\<^sub>A graph.path_weight (summ sg) (Loc nid''' (Trg p1)) (Loc nid'' (Trg p''))")
                                                    subgoal
                                                    apply (auto 0 0)

                                                    find_theorems p1 nid'''
                                                    find_theorems p'''
    ...
    find_theorems "0 < _"
                                                  ...
                                                  find_theorems "_ \<Longrightarrow> ?f _ <  ?f _" name: mono

                                                  find_theorems dataflow_topology.zero_predecessors
