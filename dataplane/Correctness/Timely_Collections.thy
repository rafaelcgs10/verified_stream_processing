...
lemma ts_alt:
  \<open>ts lxs = cimage event.time (cfilter is_Data (cset_of_llist lxs))\<close>
  unfolding ts_def using cimage_cong cin_cfilter event.case_eq_if by (metis (mono_tags, lifting))
definition "outputs_ts f xs = remdups (filter (\<lambda> t. \<not> frontier_less_equal f t) xs)"

definition "coll inps t = list_of (lmap data (lfilter (\<lambda> e. case e of Data t' d \<Rightarrow> t = t' | _ \<Rightarrow> False) inps))"
definition "fcoll inps t = (map data (filter (\<lambda> e. case e of Data t' d \<Rightarrow> t = t' | _ \<Rightarrow> False) inps))"
