...
lemma remove_non_zero_weights_only_zero: "(0::'a::{monoid_add,order}) \<in>\<^sub>A weights l1 l2 \<Longrightarrow> (0::'a) \<in>\<^sub>A remove_non_zero_weights weights l1 l2"
  ...
  by (smt (verit, best) antichain_from_list.abs_eq antichain_from_list.rep_eq antichain_from_list_def filter.simps(1,2) is_empty_antichain_empty_list
      is_empty_antichain_not_empty_list list.set_intros(1) member_antichain.rep_eq)
...
lemma no_self_loop_checker_is_graph_checker:
  ...
  unfolding no_self_loop_checker_def graph_checker_def is_empty_antichain_def 
