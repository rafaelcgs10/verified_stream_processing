...
lemma ooo_input_op_incr_op_source_op:
  ...
  (comp_op [Inr (0 :: 2, 1 :: 1) \<mapsto> Inr (1 :: 2, 1 :: 1)] buf (my_ooo_input_op os1) (my_incr_op inc os2)))
...
proof (coinduction arbitrary: sg os1 buf os2 rule: wbisim_coinduct_upto'')
  ...
  \<and> wbisim_cong R (dataflow_op sg (map_op (case_sum id id) (case_sum id id) (comp_op [Inr (0, 1) \<mapsto> Inr (1, 1)] buf
    (my_ooo_input_op os1) (my_incr_op inc (os2\<lparr>outpu := (outpu os2)(1 := xs)\<rparr>))))) op2'"
        (is \<open>\<exists>_. _ \<and> wbisim_cong _ (dataflow_op _ (map_op _ _ (comp_op _ _ _ (my_incr_op _ ?os2')))) _\<close>)
  ...
  \<and> wbisim_cong R (dataflow_op sg (map_op (case_sum id id) (case_sum id id) (comp_op [Inr (0, 1) \<mapsto> Inr (1, 1)] (BENQ (Inr (1, 1)) (Inr (d, t)) buf)
    (my_ooo_input_op (os1\<lparr>outpu := (outpu os1)(1 := xs)\<rparr>)) (my_incr_op inc os2)))) op2'"
  ...
  \<and> wbisim_cong R (dataflow_op sg (map_op (case_sum id id) (case_sum id id) (comp_op [Inr (0, 1) \<mapsto> Inr (1, 1)] (BTL (Inr (1, 1)) buf)
    (my_ooo_input_op os1) \<oslash>))) op2'"
      ...
      moreover have "\<exists>op2'. (step Tau)\<^sup>*\<^sup>* (my_source_op f inc os1 buf os2) op2'
  \<and> wbisim_cong R (dataflow_op sg (map_op (case_sum id id) (case_sum id id) (comp_op [Inr (0, 1) \<mapsto> Inr (1, 1)] (BTL (Inr (1, 1)) buf)
    (my_ooo_input_op os1) (my_incr_op inc (consumes os2 1 t d))))) op2'"
        (is \<open>\<exists>_. _ \<and> wbisim_cong _ (dataflow_op _ (map_op _ _ (comp_op _ ?buf' _ (my_incr_op _ ?os2')))) _\<close>)
        if "invariant f inc os1 buf os2"
          and "buf (Inr (1, 1)) \<noteq> []"
  ...
  = (d, t + inc) # map ((\<lambda>(d, t). (d, t + inc)) \<circ> projr) (BTL (Inr (1, 1)) buf (Inr (1, 1)))\<close>
      ...
      moreover have "\<exists>op2'. (step Tau)\<^sup>*\<^sup>* (my_source_op f inc os1 buf os2) op2'
  \<and> wbisim_cong R (dataflow_op sg (map_op (case_sum id id) (case_sum id id) (comp_op [Inr (0, 1) \<mapsto> Inr (1, 1)] buf
  (my_ooo_input_op os1') (my_incr_op inc os2)))) op2'"
        if "invariant f inc os1 buf os2"
  ...
  (my_ooo_input_op os1) (my_incr_op inc os2')))) op2'"
        if "invariant f inc os1 buf os2"
          and "os2' |\<in>| incr_op_logic 1 1 inc os2"
      ...
      proof -
        have outpu_os2': \<open>outpu os2' 1 = outpu os2 1 @ map (\<lambda>(d, t). (d, t + inc)) (input os2 1)\<close>
          using that(2) unfolding trace_simp incr_op_logic_def drop_caps_def release_caps_def produces_def by (simp split: prod.splits if_splits)
        have input_os2': \<open>input os2' 1 = []\<close> using that(2) unfolding incr_op_logic_def by (simp split: prod.splits if_splits)
        have \<open>my_source_op f inc os1 buf os2 = my_source_op f inc os1 buf os2'\<close>
        ...
        moreover have \<open>invariant f inc os1 buf os2'\<close> using that unfolding invariant_def
            incr_op_logic_def release_caps_def drop_caps_def produces_def enum_num1_def by (simp add: comp_def split: prod.splits if_splits)
        ultimately show ?thesis unfolding R_def by blast
      qed
      moreover have "\<exists>op2'. (step Tau)\<^sup>*\<^sup>* (my_source_op f inc os1 buf os2) op2'
  ...
  (my_ooo_input_op os1') (my_incr_op inc os2)))) op2'"
      ...
      ultimately show ?thesis unfolding R_def[symmetric]
        by (sim_cases sim: SIM1
            defs: my_ooo_input_op_def ooo_input_op_def my_incr_op_def incr_op_def
            elims: step_dataflow_op_elim step_map_op_elim step_comp_op_elim step_builder_op_elim
            intros: invariant_initia
            simps: ooo_input_op_logic_front_initia incr_op_logic_front_initia
...
next
  ...
  proof -
    ...
    (comp_op [Inr (0, 1) \<mapsto> Inr (1, 1)] buf (my_ooo_input_op os1) (my_incr_op inc os2)))) op2'
  ...
  (dataflow_op sg (map_op (case_sum id id) (case_sum id id) (comp_op [Inr (0, 1) \<mapsto> Inr (1, 1)] buf
    (my_ooo_input_op os1) (my_incr_op inc os2))))
  (dataflow_op sg (map_op (case_sum id id) (case_sum id id) (comp_op [Inr (0, 1) \<mapsto> Inr (1, 1)] buf
    (my_ooo_input_op ?os1_2) (my_incr_op inc os2))))\<close> unfolding my_ooo_input_op_def by fast
  ...
  (dataflow_op sg (map_op (case_sum id id) (case_sum id id) (comp_op [Inr (0, 1) \<mapsto> Inr (1, 1)] buf
    (my_ooo_input_op ?os1_2) (my_incr_op inc os2))))
  (dataflow_op sg (map_op (case_sum id id) (case_sum id id) (comp_op [Inr (0, 1) \<mapsto> Inr (1, 1)] (BENQ (Inr (1, 1)) (Inr (d, t')) buf)
    (my_ooo_input_op ?os1_3) (my_incr_op inc os2))))\<close> using initia_os1_2 outpu_os1_2
            proof -
              have \<open>step (Out (Some 1) (Inr (d, t')))
  (ooo_input_op {|1 :: 1|} ?os1_2) (ooo_input_op {|1 :: 1|} ?os1_3)\<close>
                by (rule step_ooo_input_op_Write_Some[OF refl _ outpu_os1_2 refl]) simp
              hence \<open>step (Out (Inr (0, 1)) (Inr (d, t')))
  (my_ooo_input_op ?os1_2) (my_ooo_input_op ?os1_3)\<close>
                unfolding my_ooo_input_op_def by (rule step_map_op) simp
              hence \<open>step Tau
  (comp_op [Inr (0, 1) \<mapsto> Inr (1, 1)] buf (my_ooo_input_op ?os1_2) (my_incr_op inc os2))
  (comp_op [Inr (0, 1) \<mapsto> Inr (1, 1)] (BENQ (Inr (1, 1)) (Inr (d, t')) buf)
    (my_ooo_input_op ?os1_3) (my_incr_op inc os2))\<close>
                by (rule step_Tau_comp_op_L_alt) simp
              hence \<open>step Tau
  ...
  (map_op (case_sum id id) (case_sum id id)
    (comp_op [Inr (0, 1) \<mapsto> Inr (1, 1)] (BENQ (Inr (1, 1)) (Inr (d, t')) buf)
      (my_ooo_input_op ?os1_3) (my_incr_op inc os2)))\<close>
                by (rule step_map_op) simp
              thus ?thesis
                by (rule step_Tau_dataflow_op_Tau_intro)
            qed
            let ?os2_1 = \<open>consumes os2 1 t' d\<close>
            have step_Tau_2: \<open>step Tau
  (dataflow_op sg (map_op (case_sum id id) (case_sum id id) (comp_op [Inr (0, 1) \<mapsto> Inr (1, 1)] (BENQ (Inr (1, 1)) (Inr (d, t')) buf)
    (my_ooo_input_op ?os1_3) (my_incr_op inc os2))))
  (dataflow_op sg (map_op (case_sum id id) (case_sum id id) (comp_op [Inr (0, 1) \<mapsto> Inr (1, 1)] buf
    (my_ooo_input_op ?os1_3) (my_incr_op inc ?os2_1))))\<close> using that(1) buf_Nil
              unfolding invariant_def my_incr_op_def incr_op_def
              by (auto intro!: step_Tau_dataflow_op_Tau_intro)
            obtain os2_2 where os2_2: \<open>os2_2 |\<in>| incr_op_logic 1 1 inc ?os2_1\<close>
              unfolding incr_op_logic_def trace_simp Let_def 
              using inv invariant_def by force
            hence initia_os2_2: \<open>initia os2_2\<close> using that(1) unfolding invariant_def incr_op_logic_def
                consumes_def add_caps_def drop_caps_def produces_def by simp
            have input_os2_2: \<open>input os2_2 1 = []\<close> using os2_2 unfolding incr_op_logic_def by (simp split: prod.splits if_splits)
            have outpu_os2_2: \<open>outpu os2_2 1 = [(d, t)]\<close> using outpu_os2_Nil input_os2_Nil os2_2 t'_d'(2)
              unfolding incr_op_logic_def consumes_def add_caps_def drop_caps_def produces_def
            ...
            have summar_os2_2: \<open>intsum os2_2 1 1 = [inc]\<close> using that(1) os2_2 unfolding invariant_def
                incr_op_logic_def consumes_def add_caps_def drop_caps_def produces_def by simp
            have ocaps_os2_2: \<open>ocaps os2_2 1 = []\<close> using os2_2 unfolding incr_op_logic_def
                consumes_def add_caps_def drop_caps_def produces_def enum_num1_def by (simp add: comp_def split: prod.splits if_splits)
            have \<open>step Tau (my_incr_op inc ?os2_1) (my_incr_op inc os2_2)\<close> using that(1)
                Cons os2_2 unfolding invariant_def my_incr_op_def incr_op_def consumes_def
                add_caps_def by (auto intro!: step_builder_op_Silent
                  simp add: incr_op_logic_front_initia
                  operator_state_front_initia_upd_collapse)
            hence step_Tau_3: \<open>step Tau
  (dataflow_op sg (map_op (case_sum id id) (case_sum id id) (comp_op [Inr (0, 1) \<mapsto> Inr (1, 1)] buf
    (my_ooo_input_op ?os1_3) (my_incr_op inc ?os2_1))))
  (dataflow_op sg (map_op (case_sum id id) (case_sum id id) (comp_op [Inr (0, 1) \<mapsto> Inr (1, 1)] buf
    (my_ooo_input_op ?os1_3) (my_incr_op inc os2_2))))\<close> by auto
            let ?os2_3 = \<open>os2_2\<lparr>outpu := (outpu os2_2)(1 := [])\<rparr>\<close>
            have \<open>step (Out (Inr (1, 1)) (Inr (d, t))) (my_incr_op inc os2_2) (my_incr_op inc ?os2_3)\<close>
              using that outpu_os2_Nil input_os2_Nil Cons t'_d'(2) initia_os2_2 outpu_os2_2
              unfolding invariant_def my_incr_op_def incr_op_def by auto
            hence \<open>step (Out (1, 1) (d, t))
  (dataflow_op sg (map_op (case_sum id id) (case_sum id id) (comp_op [Inr (0, 1) \<mapsto> Inr (1, 1)] buf
    (my_ooo_input_op ?os1_3) (my_incr_op inc os2_2))))
  (dataflow_op sg (map_op (case_sum id id) (case_sum id id) (comp_op [Inr (0, 1) \<mapsto> Inr (1, 1)] buf
    (my_ooo_input_op ?os1_3) (my_incr_op inc ?os2_3))))\<close> by auto
            hence \<open>wstep (Out (1, 1) (d, t))
  (dataflow_op sg (map_op (case_sum id id) (case_sum id id) (comp_op [Inr (0, 1) \<mapsto> Inr (1, 1)] buf
    (my_ooo_input_op os1) (my_incr_op inc os2))))
  (dataflow_op sg (map_op (case_sum id id) (case_sum id id) (comp_op [Inr (0, 1) \<mapsto> Inr (1, 1)] buf
    (my_ooo_input_op ?os1_3) (my_incr_op inc ?os2_3))))\<close> using step_Taus step_Tau_1 step_Tau_2
  ...
  (dataflow_op sg (map_op (case_sum id id) (case_sum id id) (comp_op [Inr (0, 1) \<mapsto> Inr (1, 1)] buf
    (my_ooo_input_op os1) (my_incr_op inc os2))))
  (dataflow_op sg (map_op (case_sum id id) (case_sum id id) (comp_op [Inr (0, 1) \<mapsto> Inr (1, 1)] (BENQ (Inr (1, 1)) (Inr (d, t')) buf)
    (my_ooo_input_op ?os1') (my_incr_op inc os2))))\<close> using that(1) Cons t'(2)
  ...
  (dataflow_op sg (map_op (case_sum id id) (case_sum id id) (comp_op [Inr (0, 1) \<mapsto> Inr (1, 1)] (BENQ (Inr (1, 1)) (Inr (d, t')) buf)
    (my_ooo_input_op ?os1') (my_incr_op inc os2))))
  (dataflow_op sg (map_op (case_sum id id) (case_sum id id) (comp_op [Inr (0, 1) \<mapsto> Inr (1, 1)] buf
    (my_ooo_input_op ?os1') (my_incr_op inc ?os2_1))))\<close> using that(1) buf_Nil
              unfolding invariant_def my_incr_op_def incr_op_def
              by (auto intro!: step_Tau_dataflow_op_Tau_intro)
            obtain os2_2 where os2_2: \<open>os2_2 |\<in>| incr_op_logic 1 1 inc ?os2_1\<close>
              unfolding incr_op_logic_def invariant_def trace_simp
            ...
            hence step_Tau_3: \<open>step Tau
  (dataflow_op sg (map_op (case_sum id id) (case_sum id id) (comp_op [Inr (0, 1) \<mapsto> Inr (1, 1)] buf
    (my_ooo_input_op ?os1') (my_incr_op inc ?os2_1))))
  (dataflow_op sg (map_op (case_sum id id) (case_sum id id) (comp_op [Inr (0, 1) \<mapsto> Inr (1, 1)] buf
    (my_ooo_input_op ?os1') (my_incr_op inc os2_2))))\<close> by auto
            let ?os2_3 = \<open>os2_2\<lparr>outpu := (outpu os2_2)(1 := [])\<rparr>\<close>
            have \<open>step (Out (Inr (1, 1)) (Inr (d, t))) (my_incr_op inc os2_2) (my_incr_op inc ?os2_3)\<close>
            ...
            hence \<open>step (Out (1, 1) (d, t))
  (dataflow_op sg (map_op (case_sum id id) (case_sum id id) (comp_op [Inr (0, 1) \<mapsto> Inr (1, 1)] buf
    (my_ooo_input_op ?os1') (my_incr_op inc os2_2))))
  (dataflow_op sg (map_op (case_sum id id) (case_sum id id) (comp_op [Inr (0, 1) \<mapsto> Inr (1, 1)] buf
    (my_ooo_input_op ?os1') (my_incr_op inc ?os2_3))))\<close> by auto
            hence \<open>wstep (Out (1, 1) (d, t))
  (dataflow_op sg (map_op (case_sum id id) (case_sum id id) (comp_op [Inr (0, 1) \<mapsto> Inr (1, 1)] buf
    (my_ooo_input_op os1) (my_incr_op inc os2))))
  (dataflow_op sg (map_op (case_sum id id) (case_sum id id) (comp_op [Inr (0, 1) \<mapsto> Inr (1, 1)] buf
    (my_ooo_input_op ?os1') (my_incr_op inc ?os2_3))))\<close> using step_Tau_1 step_Tau_2 step_Tau_3
              by fast
            moreover have \<open>map_op (\<lambda>(p :: 1). (1, 1)) (\<lambda>(p :: 1). (1, 1)) (source_op ((\<lambda>(p :: 1). LCons (d, t) lxs)(1 := lxs)))
  ...
  (dataflow_op sg (map_op (case_sum id id) (case_sum id id) (comp_op [Inr (0, 1) \<mapsto> Inr (1, 1)] buf
    (my_ooo_input_op os1) (my_incr_op inc os2))))
  (dataflow_op sg (map_op (case_sum id id) (case_sum id id) (comp_op [Inr (0, 1) \<mapsto> Inr (1, 1)] ?buf'
    (my_ooo_input_op os1) (my_incr_op inc ?os2_1))))\<close>  using that(1) Cons t'(2)
            unfolding invariant_def my_incr_op_def incr_op_def
            by (auto intro!: step_Tau_dataflow_op_Tau_intro)
          obtain os2_2 where os2_2: \<open>os2_2 |\<in>| incr_op_logic 1 1 inc ?os2_1\<close>
            unfolding incr_op_logic_def invariant_def trace_simp
          ...
          hence initia_os2_2: \<open>initia os2_2\<close> using that(1) unfolding invariant_def incr_op_logic_def
              consumes_def add_caps_def drop_caps_def produces_def by simp
          have input_os2_2: \<open>input os2_2 1 = []\<close> using os2_2 unfolding incr_op_logic_def by (simp add: comp_def split: prod.splits if_splits)
          have outpu_os2_2: \<open>outpu os2_2 1 = [(d, t)]\<close> using outpu_os2_Nil input_os2_Nil os2_2 t'(1)
            unfolding incr_op_logic_def consumes_def add_caps_def drop_caps_def produces_def
  ...
  (dataflow_op sg (map_op (case_sum id id) (case_sum id id) (comp_op [Inr (0, 1) \<mapsto> Inr (1, 1)] buf
    (my_ooo_input_op os1) (my_incr_op inc os2))))
  (dataflow_op sg (map_op (case_sum id id) (case_sum id id) (comp_op [Inr (0, 1) \<mapsto> Inr (1, 1)] ?buf'
    (my_ooo_input_op os1) (my_incr_op inc ?os2_3))))\<close> using step_Tau_1 step_Tau_2 by fast
          moreover have \<open>map_op (\<lambda>(p :: 1). (1, 1)) (\<lambda>(p :: 1). (1, 1)) (source_op ((\<lambda>(p :: 1). LCons (d, t) lxs)(1 := lxs)))
  = my_source_op f inc os1 ?buf' ?os2_3\<close> using that outpu_os2_Nil input_os2_Nil Cons input_os2_2
      ...
      next
        case (Cons _ xs)
        obtain os2' where os2': \<open>os2' |\<in>| incr_op_logic 1 1 inc os2\<close>
          unfolding incr_op_logic_def invariant_def trace_simp Let_def
          using inv invariant_def local.Cons by force
        hence initia_os2': \<open>initia os2'\<close> using that(1) unfolding invariant_def incr_op_logic_def
            drop_caps_def produces_def by (simp add: comp_def split: prod.splits if_splits)
        have input_os2': \<open>input os2' 1 = []\<close> using os2' unfolding incr_op_logic_def by (simp add: comp_def split: prod.splits if_splits)
        have outpu_os2': \<open>outpu os2' 1 = map (\<lambda>(d, t). (d, t + inc)) (input os2 1)\<close>
          using outpu_os2_Nil os2' unfolding incr_op_logic_def drop_caps_def produces_def
          by (simp add: comp_def split: prod.splits if_splits)
        have summar_os2': \<open>intsum os2' 1 1 = [inc]\<close> using that(1) os2' unfolding invariant_def
            incr_op_logic_def consumes_def add_caps_def drop_caps_def produces_def by (simp add: comp_def split: prod.splits if_splits)
        have ocaps_os2': \<open>ocaps os2' 1 = []\<close> using os2' unfolding incr_op_logic_def
            consumes_def add_caps_def drop_caps_def produces_def enum_num1_def by (simp add: comp_def split: prod.splits if_splits)
        have \<open>step Tau (my_incr_op inc os2) (my_incr_op inc os2')\<close> using that(1) Cons os2'
          unfolding invariant_def my_incr_op_def incr_op_def
  ...
  (dataflow_op sg (map_op (case_sum id id) (case_sum id id) (comp_op [Inr (0, 1) \<mapsto> Inr (1, 1)] buf
    (my_ooo_input_op os1) (my_incr_op inc os2))))
  (dataflow_op sg (map_op (case_sum id id) (case_sum id id) (comp_op [Inr (0, 1) \<mapsto> Inr (1, 1)] buf
    (my_ooo_input_op os1) (my_incr_op inc os2'))))\<close> by auto
        let ?os2'' = \<open>os2'\<lparr>outpu := (outpu os2')(1 := tl (outpu os2' 1))\<rparr>\<close>
        have \<open>step (Out (Inr (1, 1)) (Inr (d, t))) (my_incr_op inc os2') (my_incr_op inc ?os2'')\<close>
          using that outpu_os2_Nil Cons initia_os2' outpu_os2' unfolding invariant_def my_incr_op_def
            incr_op_def by auto
  ...
  (dataflow_op sg (map_op (case_sum id id) (case_sum id id) (comp_op [Inr (0, 1) \<mapsto> Inr (1, 1)] buf
    (my_ooo_input_op os1) (my_incr_op inc os2))))
  (dataflow_op sg (map_op (case_sum id id) (case_sum id id) (comp_op [Inr (0, 1) \<mapsto> Inr (1, 1)] buf
    (my_ooo_input_op os1) (my_incr_op inc ?os2'))))\<close> using that Cons unfolding invariant_def
        my_incr_op_def incr_op_def by (auto intro!: step_wstep)
      moreover have \<open>map_op (\<lambda>(p :: 1). (1, 1)) (\<lambda>(p :: 1). (1, 1)) (source_op ((\<lambda>(p :: 1). LCons (d, t) lxs)(1 := lxs)))
