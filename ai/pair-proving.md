...
- **Timeouts**: never wait more than 10 seconds for a single command via MCP (`timeout_per_command ≤ 10000`). If a `by auto`/`by metis`/etc. doesn't return in that window, treat it as divergent — cancel and try a more targeted tactic instead of waiting longer.
